...
- **Design taste:** when generating or restyling dashboard/storefront UI (including storegen prompt/lever work), apply the `design-taste-frontend` skill (taste-skill v2 — installed with `npx skills add Leonxlnx/taste-skill` into `.agents/skills/`) to avoid generic output; it composes with `emil-design-eng`. If the experimental v2 misbehaves, use `design-taste-frontend-v1`.
