...
### Triage — window = prior tip `495af9a` → now `530ed58`. EXACTLY ONE PR landed: **#659** (John Duncan, merge `530ed58`, commit `1f8e257` — *"github-digest: env-driven brand + repo (DIGEST_BRAND)"*). NOTE topology gotcha: the 07-24 run recorded main still at `495af9a`; this run had to `git fetch origin main` — origin/main had advanced to `530ed58` but the local checkout was detached at the fetched merge. Always re-fetch main before triage; don't trust the prior night's recorded tip.
- **#659 diff (7 files, +94/-37, all `app/lib/github-digest/`):** parameterizes the internal daily dev-digest email by brand. `render.server.ts` — `logoLockup()`→`logoLockup(brand)` (badge initial = `brand.charAt(0).toUpperCase()||"?"`, wordmark = `esc(brand)`); `RenderInput` gains required `brand: string`; `renderEmptyHtml(dateLabel)`→`(dateLabel, brand)`; renderText title = `input.brand.toUpperCase()`. `run.server.ts` — `brand = env.DIGEST_BRAND||"Calderyn"`, `isDefaultRepo = repo===DEFAULT_REPO`; waitlist section (Calderyn-specific table) now SKIPPED with a note when repo ≠ default. `summarize.server.ts` — `SYSTEM_PROMPT` const → `systemPrompt(brand)` fn; subject = `${brand} dev digest — ${date}`; threads brand into both RenderInput builds. Tests all updated in-commit (input helper carries `brand:"Calderyn"`; new custom-brand + renderEmptyHtml tests).
- **Verdict: CLEAN + well-typed + fully self-tested. NO independent landed bug → no fixers, no fix branch, no `npm ci`, NO fix PR** (Phase 4: no bug WE fixed ⇒ no PR). Same all-clean-landed shape as 07-20 / 07-22 / 07-24. Repo left clean on detached `530ed58` (== origin/main).

### The ONE imperfection in #659 (borderline nit) was ALREADY FIXED by the companion — do NOT re-flag, do NOT open our own fix for it
`render.server.ts` footer hardcodes `REPO_URL = .../calderyn-shopify-app` (HTML `:251` + text `:284`); it is NOT threaded from `DIGEST_REPO`, so a re-pointed digest (`DIGEST_REPO=Mezoh/hive-mind`) still links + labels the Calderyn repo. Header wordmark/subject ARE repointed, footer link is NOT → mild inconsistency, only in the non-default config, internal ops email, purely cosmetic. Sits right at the CONSERVATIVE bar. The existing custom-brand render test only asserts `not.toContain("Calderyn")` (capital-C) so the lowercase `calderyn-shopify-app` slug slips through — the test does NOT catch it. **Companion #661 fixed exactly this (its finding #1)** → we do NOT duplicate.

### Open-PR landscape — now FOUR overlapping nightly PRs against/around this window; companion **#661** is THIS window's genuine companion
- **#661** (Mezoh, "Nightly review fixes — 2026-07-25", base `530ed58` = current tip, non-draft, created 04:24Z) — reviewed the SAME #659 window. 5 fixes, gate-green (its body: npm ci · typecheck · lint · build · targeted tests all ✅, "Verified 2864 client files"). Reviewed its full diff (9 files, +96/-15) against source — **ALL 5 GENUINE, minimal, correct, no regressions, no bugs introduced → NONE, posted NO comment** (Phase 3 posts only on bug findings + frugal rule):
  1. **footer-repo fix** — new `repoFooter(repo?)` in render.server.ts derives `{url,label}` from the slug (`slug.split("/").pop()`), threads `repo?` through `RenderInput`→`summarize`→`run.server` (`repo = env.DIGEST_REPO||DEFAULT_REPO`), now `esc()`-wrapped, defaults to Calderyn repo. + 2 tests (repointed + default). CLEAN — this is the #659 nit above.
  2. **PR-search truncation note** (`collect.server.ts`) — `searchPRs` now returns `{items, totalCount}` (from `total_count`); `collectActivity` pushes `"Showing N of M merged/opened pull requests (capped at 50)…"` when truncated (rule-12 parity with the branch-scan cap). Genuinely NEW (found by broadening beyond the #659 diff), CLEAN. (Minor: the "opened" note uses pre-dedup `items.length`, not the post-merged-filter display count — honest about the *search* cap, not a bug.)
  3/4/5. **DUPLICATES of already-open #657/#658** — `route`+`ref` added to `OrderListFamily ASC_FIRST_SORT_COLS` (does NOT also remove `transfer` the way #658 does — not a new bug, just a narrower edit); discover short-query fix via whole-trimmed-query fallback (the #657-style impl, not #658's per-token); onboarding loader `listDiscoverFeed` try/catch→`[]`. Verified `DiscoverFeedItem` IS imported in `dashboard.onboarding.tsx:34` (from `~/lib/sourcing/types`) → the new `let products: DiscoverFeedItem[]` annotation typechecks.
- **COORDINATION FLAG (carry forward):** #661, #657, #658 now make CONFLICTING/overlapping edits to the SAME files — `app/lib/sourcing/discover.server.ts` (short-query fix, 3 different-but-overlapping impls), `app/routes/dashboard.onboarding.tsx` (loader guard), `app/components/dashboard/screens/OrderListFamily.tsx` (ASC cols). Plus our draft #656. Only ONE of the discover/onboarding sets can merge cleanly; the rest will conflict. This is a maintainer merge-order decision, NOT a bug. **#661's #3/#4/#5 are re-fixes of the 07-22 landed batch already on #657/#658 — do NOT re-triage them as new landed bugs next night. The only items UNIQUE to this window's #659 are #661's #1 (footer) + #2 (PR-search truncation).**
- **#656** (ours, keyuchen, DRAFT) unchanged; **#657/#658** (Mezoh 07-23/07-24) unchanged & documented-CLEAN; **#642** (07-22) / **#608** (campaign sales-profit) unchanged & documented-CLEAN. Re-reviewing unchanged cleared PRs = noise → skipped all.

### Gate / environment
- No gate run by us (no fixers, no fix branch — nothing to gate). #661's own green gate against `530ed58` independently confirms #659's landed code compiles + its tests pass (no hidden landed test-gate regression — the 07-23 stale-test family did NOT recur; #659's tests were updated in-commit and are consistent with source). Prior baselines stand (15 total vitest failures on clean main: 8 environmental no-Chrome/no-Shopify-env + the login-button set; CI billing outage still assumed).
- Notify sent (coordination: all-clean landed night; #659 clean; companion #661 covers #659's only nit + adds a PR-truncation note; FOUR overlapping nightly PRs, #661/#657/#658 conflict on discover.server.ts + onboarding.tsx — maintainer merge-order call).
...
Window clusters: **Sortable column headers (BIG new feature, John)** — server RPCs `po_list`/`inventory_list` (migrations `po_list_sortable`/`inventory_list_fn_v3`) + client comparators `po-sort.ts`/`campaign-sort.ts`/`catalog/inventory-sort.ts` across Campaigns/Discover/OperatingPnl/Payments/Transfers/Inventory/PurchaseOrders/OrderListFamily; **Multi-account login chooser (John, dbe0eb3/90e5273/d3fe08f + b204497)** — `remembered-accounts.server.ts` [NEW], `switch-account`/`logout` routes, `AccountChooser.tsx`, login/signup rewrite, phone-optional onboarding; **Payments/orders (John)** — payout-account recovery `connect.server.ts` (0a2d713), merchant-order email + stripe webhook `merchant-notify.server.ts`/`stripe.server.ts` + journey publish-milestone (79f84a5), dropshipping import #649; **Dashboard polish** — Analytics/Lists/OperatingPnl visible-refetch state + `ui.tsx` Refetch component, Home greeting navigate, fulfillment_fact(order_id) index (267aa55). Hunters: A sorting, B auth-chooser, C payments/orders/import, D dashboard-polish/ui/migration.

### Bug #1 fixed — explicit sign-out leaves a ONE-CLICK re-entry when the server-side session revoke fails (SECURITY regression, dbe0eb3/90e5273)
`app/routes/dashboard.api.logout.tsx` + `app/lib/auth/remembered-accounts.server.ts rememberOnSignOut`. Logout's `revokeSession` is best-effort inside a swallowed try/catch ("logout must never 500"). `90e5273` changed `rememberOnSignOut` to KEEP the just-signed-out token in the `__Host-calderyn_accounts` cookie (`if (t === activeRaw) return true`) so the account stays listed in the chooser; the downgrade to a password-required (`live:false`) shortcut depends ENTIRELY on the DB row being revoked (`rowIsLive` checks `revoked_at`/`expires_at`). If `revokeSession` THROWS (transient Supabase failure — exactly what the catch exists for), the row stays LIVE, the token is retained, `activateRememberedAccount` returns `{ok:true}`, and `switch-account` mints a full session with NO re-auth → violates logout's own documented invariant. REGRESSION: the prior `dbe0eb3` impl always dropped `activeRaw` (`tokens.filter(t=>t!==activeRaw)`), so a failed revoke left no one-click path. Fix (commit `058defe`): `logout.tsx` tracks `revoked` (true when no live session OR revoke completed; false only when a live session's revoke threw) and passes it to `rememberOnSignOut(request, activeRaw, revoked)`; when `!revoked`, drop `activeRaw` from the cookie (safe pre-90e5273 behavior). +2 regression tests.
- **Lesson (NEW family — "best-effort side effect gates a security invariant"): when a swallowed/best-effort operation (a revoke, a delete, a flag flip) is the ONLY thing that downgrades a retained credential/token from live→dead, its failure silently breaks the invariant. Whenever a change moves from "drop the token" to "keep it listed, rely on the row being dead," verify the row-death is guaranteed — not best-effort. Grep for retained tokens/creds whose safety depends on a `try{...}catch{}`-swallowed write.**

### Bug #2 fixed — landed TEST-GATE regression: stale login/signup provider-button tests (dbe0eb3)
`app/routes/__tests__/login-google-button.test.tsx` (5) + `login-shopify-button.test.tsx` (2) were RED on `main`. `dbe0eb3` gave `login.tsx:75`/`signup.tsx:72` a new `accounts` loader field (`const hasAccounts = accounts.length > 0`) and switched the loaders to `return json({...})`, but did NOT update these pre-existing guard tests (they assert the Google/Shopify buttons point at the APEX host so the `__Host-` CSRF cookie + redirect_uri line up). The render mocks omit `accounts` → `undefined.length` throws in `renderToStaticMarkup`; the loader test does `toMatchObject` on what is now a `Response`. SOURCE IS CORRECT (loader always supplies `accounts` in prod); tests were stale. Fix (commit `6722bef`, test-only): add `accounts: []` to the 6 render mocks (no-remembered-accounts state), unwrap the loader Response with `await res.json()`.
- **Lesson (REINFORCES 2026-07-18 Request-equality + 2026-07-15 #3): a window that rewrites an auth/loader route (here login/signup) routinely leaves sibling guard tests stale, and the CI billing outage lets them sit RED on `main`. TWO stale-test tells to grep for when a loader changes: (a) a render-test mock that omits a NEWLY-ADDED loader field → `undefined.length`/`.map` throws inside renderToStaticMarkup; (b) a loader unit test doing `toMatchObject`/`.field` on the raw return after the loader switched to `json()`/`redirect()` (now a Response) → needs `await res.json()`. ALWAYS run the full `npm test` when a `.tsx` route loader/component is rewritten in-window — do not trust the "8 environmental" baseline blindly; diff the failing-file SET against the known 4 environmental files.**

### False positives cleared / verified CLEAN this run (do NOT re-flag)
- **Sortable column headers (03b8481/d04e207/65ee1db) — CLEAN.** THE #1 tiebreak family HELD: both RPCs cap `limit ≤ 100` (< 1000 clamp) and every ORDER BY branch (incl. defaults + new CASE arms) ends on a unique tail — `po.created_at desc, po.id desc` and `r.product_title asc, r.variant_id asc` (variant_id unique since `rolled` groups by v.id); nullable cols carry `nulls last`; `count(*) over ()` is pre-LIMIT (honest totals). Sort keys matched against static CASE-arm literals (no dynamic SQL); server maps request key via `parsePoSort`/`parseInventorySort` allowlists, unknown → default; `p_dir` normalized asc/desc; tenant from `session.shopId`. Client comparators all `[...items].sort()` (copy-first, no loader-array mutation), numeric cols use numeric subtraction, ISO dates `localeCompare`, nulls sink last both dirs, stable tiebreak each; shared `nextSortState` = new-col→natural, same-col→flip, 3rd→default. The 5 client-sorted screens don't paginate; the 2 RPC-paged screens refetch from offset 0 on sort change w/ a stale-page filter token. Supabase errors thrown not swallowed.
- **Payments/orders/import (0a2d713/79f84a5/d8da15a) — CLEAN.** Merchant email is inside the `if (processed)` first-delivery branch of `processStripeEvent` (gated by `record_stripe_event` RPC dedup) → Stripe retries deliver `processed=false`, no dup email; `sendMerchantNewOrderNotice` wraps its body in try/catch returning `{sent:false,error}` (a mailer outage can't 500 the webhook / trigger retry storm), while upstream Stripe/order errors still throw. Payout recovery: `isUnknownAccountError` matches `account_invalid`/not-owned; re-provision only for a never-onboarded row (onboarded rows refused via `PayoutAccountError`→502), tenant-scoped, no loop. Journey milestone adds `published_version_id != null` OR `designer_publications > 0` (throwing count helpers, no swallow), doesn't false-flip. Dropshipping import: ids deduped, `>0 && <=100`, capped 8, shop-scoped `pickProduct`, `completeOnboarding` only after all picks, feed clamped 200 then slice.
- **Dashboard polish + ui.tsx + migration (9ec8880/228162e/488b293/267aa55) — CLEAN.** NEW `Refetch` component in `ui.tsx` takes NO `...rest` but NO caller passes `data-*`/`aria-*` to it (3 call sites pass only active/className/children) — so NO passthrough-swallow (the #642 Card family does NOT recur here; it renders `aria-busy={active||undefined}` correctly). Refetch-state driven by existing `loading` flag, set at fetch entry + reset in `.finally` (can't get stuck); warmup effects one-shot, cache-only. Home greeting = real `<button type=button onClick>`, `sublineTo` maps to valid nav ids, plain variants → null. Migration `create index if not exists fulfillment_fact_order_id_idx ... (order_id)` — NOT CONCURRENTLY (no in-txn hazard), idempotent; trailing `alter role authenticator set statement_timeout='30s'` idempotent, flagged revert-me.

### Gate / environment
- **Baseline gotcha (IMPORTANT correction): this window's clean `main` tip `495af9a` = 15 test failures, NOT 8.** 8 are the stable environmental set (6× `storefront-validation/browser.server.test.ts`, 1× `storefront-command/command.server.test.ts`, 1× `storefront-recipes/diagnostic-deck/bundle.test.ts` — no-Chrome; 1× `ingest/product-facts.test.ts` — missing Shopify env), same 4 files as 07-22. The OTHER 7 (`login-google-button.test.tsx` ×5 + `login-shopify-button.test.tsx` ×2) were a LANDED regression (Bug #2), NOT environmental. **Do not assume "8 environmental" without enumerating the failing FILE SET — a real regression hid in the count this night.** After both fixes: setup 0 · typecheck 0 · lint 0 (7 pre-existing warnings, untouched files) · build 0 (client-bundle verifier passed) · **vitest = 8 failed (the environmental set) | 7870 passed | 12 skipped** (7863 baseline + 7 login-button now green).
- **npm ci ran ONCE by orchestrator (exit 0), reused by both sequential fixers.** Two fixers dispatched SEQUENTIALLY (never parallel — shared checkout): #1 logout (058defe), #2 test-fix built on top (6722bef). The "don't launch baseline `npm test` while hunters run" gotcha was honored — ran only targeted login-button files after hunters finished; no machine-load flakes observed.
- **CI billing outage STILL present on #656 — SAME signature.** Both `Node gate` (89128677402) and `Python engine tests` (89128677350) `failure` with output summary/text/title ALL "" (runner never executed); Python failing on a pure-TS/test-only diff = impossible from code. Posted ONE reply on #656 explaining it (drive-to-green posture requires a fix OR a reply; this is the base-branch-infra reply). Vercel preview = Building (deploys fine). #656 auto-subscribed — treat CI-failure + vercel-Building webhooks as expected noise.
- **Open PRs this run: #642 + #608, BOTH unchanged since prior nights & documented CLEAN → posted NO comment (Phase 3 no-op).** #642 = the PREVIOUS window's companion (radar fixes, updated 2026-07-22T05:27, base `06872c6` — its 6 items are last window's landed defects, NOT this window; do NOT re-triage). #608 = campaign sales-profit, unchanged since 2026-07-21 (documented CLEAN). Re-reviewing unchanged already-cleared PRs = pure noise; skipped per the "be frugal on GitHub" rule.

## 2026-07-22

### Triage — window = base `7260303` (last night's tip, merge of #616) → `06872c6`; 140 commits (~63 non-merge to triage), dominated by the NEW **Radar** competitor-intelligence subsystem. Fanned out 4 read-only hunters; ALL 4 CLEAN. Companion #642 fixed 6 genuine bugs → posted ONE cross-check comment, did NOT open a duplicate fix PR.
Window clusters: **Radar competitor subsystem (BIG new feature, John Duncan)** — feat/radar #622 + follow-ons #627/#630/#637: SSRF-hardened polite fetcher (`app/lib/radar/fetch*` DNS-resolve + pinned-egress undici dispatcher + per-hop redirect re-check + encoding-agnostic embedded-IPv4), competitor/snapshot persistence + hash-gated diff (`snapshot.server.ts`), web_search auto-discovery quota-gated (`discovery.server.ts`), price/positioning detectors (`detect-competitors.server.ts`), apply/revert CAS orchestrator, resumable crons w/ cursor fairness, radar UI folded into Autopilot+Analytics-Live (`radar-sections.tsx`, `dashboard.api.radar.tsx`); **Designer imagery flagship (John)** — image-registry validator+repair+publish-gate, mandatory-hero reuse-before-generate, reserved first-build budget, gemini 429 in-slot retry; **Storefront recipes Taste-hardening (Eric Chen)** — nine CRO templates + atelier/gilt/ember/volt/glow/larder recipes, redesign-contract align, review-runtime-in-previews (mostly `.webp` noise); **Dashboard** — P&L date-grammar parse (febcc32) + QBO summarize enum + product-total reconcile, OAuth connect-return flows (62dd43e/c7a4777), Customers full-column sort, ErrorBoundary stale-deploy auto-heal, Alerts feed. Hunters: (A) Radar, (B) Designer-imagery+storegen, (C) Storefront-recipes+compiler/runtime, (D) Dashboard-analytics+OAuth+Autopilot-writes. **ALL 4 CLEAN — no landed bug found independently; the companion's 6 are the window's only defects.**

### Companion PR #642 (Mezoh, "Nightly review fixes — 2026-07-22", base==main `06872c6`, open/non-draft, gate-green) — 6 GENUINE fixes all verified by DIFF+source; NO duplicate PR, posted ONE cross-check comment (recommended merge).
SAME decision as 2026-07-20 (#602) / 2026-07-21: companion targets THIS window's own brand-new code, all fixes genuine, no staleness/conflict, gate-green → duplicating = pure noise. Verified each of #642's 6 against live source:
1. `ui.tsx Card` typed props as a CLOSED literal + rendered a bare `<div>` → any extra HTML attr (`data-explain` on 6 `OperatingPnl` Cards) was dropped → 6/7 P&L tooltips dead (only the products section worked — raw `<div>`). Fix = `& HTMLAttributes<HTMLDivElement>` + spread `...rest` (placed BEFORE the fixed role/className so it can't override). GENUINE. **NOTE/LESSON: hyphenated JSX attrs (`data-*`/`aria-*`) are EXEMPT from excess-property checks — a component that doesn't spread `...rest` silently swallows them with NO tsc error. On any component that receives `data-*`/`aria-*` passthrough, confirm it actually spreads rest onto the DOM node.**
2. `Customers.tsx:495` weather-heal `useEffect` had NO dep array → ran every render; the `.catch` resets `healedRef`, so a persistently-failing heal PUT re-fires on essentially every frame. Fix = `[navSub, guardrails, refresh]`. GENUINE.
3. `dashboard.api.radar.tsx:225` AI-assistant tile `.gte("day",since)` unbounded + `day>=cut` folds the partial current UTC day into `hitsLast7` (8 dates vs `hitsPrior7`'s 7). Fix = `.lt("day", today)`. GENUINE — same "exclude partial day" family as last night's radar-detect fix.
4. `snapshot.server.ts:44 PRICE_RE` `[$£€]\s?\d{1,3}(?:[.,]\d{3})*...` never matched a comma-less price ≥$1000 (`$1500`: `\d{1,3}` stalls at 3 digits, the `(?!\d)` tail rejects the leftover `0` → whole token fails) → invisible to ALL competitor price diffing (themes rendering prices without a thousands comma). Fix = `\d+`. GENUINE. (Side effect: now also matches a bare `$123456789` — low risk, review-only informational path.)
5. `snapshot.server.ts:145 labeledPriceChanges` `new Map(labeledPrices.map(...))` collapses a duplicated heading ("Featured" on 2 cards at different prices) to the LAST entry → fabricates a `competitor_price` was→now for a product whose price never changed. Fix = `uniqueLabeledPrices` (label must appear exactly ONCE on BOTH sides). GENUINE — violates the `priceChanges` truthfulness contract.
6. `discovery.server.ts:180` insert loop bounded by `DISCOVERY_MAX_SUGGESTIONS` (this run's inserts) while the entry gate treats that as the TOTAL pending cap → 4 existing + 5 new = 9. Fix = `remainingSlots = MAX − suggestedCount`. GENUINE (classic partial-run-vs-total cap off-by).
#642's 3 "found but not fixed" (meta-description undetectable end-to-end since `normalizeText` strips tags before hashing + `diffExtracts` has no meta field; a labeled `competitor_price` move can still ship the generic pages' unpaired `newPrices`/`removedPrices` to `draft.server.ts polish()`; loader-vs-action UUID guard inconsistency in `dashboard.api.radar.tsx`) are reasonable maintainer calls — the fact-shaping one (restrict `polish()` facts for `competitor_price` to the paired `priceChanges`) is the only one worth a focused follow-up. **If #642 is still open next night, do NOT re-triage its 6 items as new bugs — they are this window's landed defects, already fixed on its branch.**

### False positives cleared / verified CLEAN this run (do NOT re-flag) — all 4 hunter clusters
- **Radar subsystem (feat/radar #622 + follow-ons) — CLEAN.** SSRF: string-guard `isPubliclyRoutableHttps` + DNS-resolve `findBlockedAddressReason` + undici `pinnedLookup` (re-validates + pins exact IP at connect); redirects manual, same-origin-only, DNS-rechecked per hop; IPv6/embedded-IPv4-mapped/trailing-dot/CGNAT/link-local/TEST-NET/`0:...:1`-loopback all covered. Paid spend: `draftShopMoves` increments `attempts` BEFORE each `polish()`, gates `attempts < RADAR_NIGHTLY_CLAUDE_CAP`; separate quota buckets (radar=5, radar_apply=10, radar_discovery=2) each shared cron+manual so refresh-spam can't overspend; `isCoolingDown` skips existing drafts before spend. Crons: cursors stamped on success AND routine skips (can't wedge the queue head); snapshot upsert idempotent on `(competitor_id,url,captured_day)`; hash-gate skips unchanged pages. Apply/revert: `updateMove` CAS via `expectedStatus`; rawHtml splice uses a replacer FUNCTION + `escapeHtml`; apply-time hash re-reads the round-tripped jsonb (matches revert's read). NO read pages past the 1000-row clamp (all `.limit()`-bounded; heavy agg pushed into `read_radar_ranking_series`/`radar_rollup_traffic`); both drain queues order `... nulls first, s.id`. `detectTrafficDrops` truncated-top-path guard (last night's fix) HELD. Auth: every write via requireSameOrigin+requireDashboardSession, shopId from session, `isUuid` gate + surfaced Supabase errors.
- **Designer imagery flagship + storegen — CLEAN (money).** Product-photo loop gates through `applyAssetOverrides` → only enhances `images.length===0`; already-generated skipped every rebuild/resume; hero gated on `data.assets?.hero`; `adoptDesignerAsset` zero-quota. 429 retry (`gemini.server.ts:216`) is IN-SLOT inside the already-`started()` event — single reserve, `complete()` once, `release()` only when `providerStarted===false`. Reserved first-build budget raises only `perShopDaily` via the RPC limits override — no double-charge. Flag-off = zero spend (throws before `meter.reserve`). Validator/repair-loop/publish-gate are READ-ONLY string analysis (can't loop-and-repay; publish gate = hard 422).
- **Storefront recipes + compiler/runtime — CLEAN.** ALLOWED_TAGS↔guard parity: both interactive-control guards (`html.ts:282` + `validate.ts:259`) enumerate `button|details|summary|input|select`, matching every interactive tag in ALLOWED_TAGS — no drift. Per-card purchase identity stamped per-render via `authorityKeyForSlot` (`product:<id>`), NOT a global `/g` replace. Both `fetcher(...).then(refresh)` in `storefront-hydrator.tsx` (191,204) carry `.catch(()=>{})`. `render.tsx`/hydrator reference `./public-data.server` only via `import type` (erased). All 12 templates' `activeVersion` == manifest-array length (`if(!assets) throw` fail-fast). `4bc0987` replaced `requireSameOrigin` for review templates with an explicit `Origin===requestUrl.origin` check that FAILS CLOSED on missing/mismatched Origin (did NOT open cross-origin).
- **Dashboard analytics/OAuth/Autopilot-writes — CLEAN.** `operating-pnl.server.ts` `readPaged`/`.range()` over all 6 order/refund/line tables carry `.order("id",asc)` (unique tiebreak). Remainder-to-last allocator: the `revenue===0` zero-basis guard is evaluated BEFORE the `index===last` short-circuit (the 2026-07-19 bug family — correct here). QBO date-grammar parse (febcc32): `columnStartDate`+`parseQuickBooksReport` stay index-aligned with `net[]` (both slice(1)); title regex reduces range/partial-edge titles to their start day, "Total"→"" (filtered), `Date.parse` guarded by `Number.isFinite` (no throw path). OAuth: `safeDashboardReturnTo` requires `/dashboard/` prefix + rejects `//`/`\`/`://`/ctrl; `safeDashboardOAuthReturnTo`/`Origin` exact-path + env-allowlist re-validated at USE-time; GSC callback validates the CSRF state cookie. Undo: audit read `.eq("shop_id")`, guards undo-of-undo/non-succeeded/existing-undo/actor-window, inventory reversal refuses without a replay plan + re-checks fresh availability. Write actions (audit undo, alert action) = requireSameOrigin+requireDashboardSession+idempotency-key, tenant from session. ErrorBoundary auto-heal throttled 1/60s via sessionStorage, fails closed. Customers/Orders/Alerts = client-only; CSV export neutralizes formula injection (`csvCell` prefixes `=+-@`).

### Gate / environment
- **Clean-`main` `06872c6` baseline: `npm ci` 0 · typecheck 0 · full `vitest run` = 8 failed | 7793 passed | 12 skipped.** All 8 environmental: **6×** `storefront-validation/browser.server.test.ts` + **1×** `storefront-command/command.server.test.ts` + **1×** `storefront-recipes/diagnostic-deck/bundle.test.ts` (all "No local Chrome"/root-sandbox), and **1×** `app/lib/ingest/__tests__/product-facts.test.ts` (file-init fails `validateConfig` — missing `apiSecretKey`/Shopify env vars; this is the "8th file", count stable at 8 test-failures across 4 files). Matches #642's reported baseline exactly.
- **GOTCHA (machine-load flake — NEW): a FIRST full `npm test` launched WHILE 4 hunter subagents + typecheck were concurrently running took 209s (vs ~110-120s quiet) and flaked 2 EXTRA failures in `app/routes/__tests__/dashboard-index-loader.test.ts` — one `Test timed out in 5000ms`, one `productTourPending: expected true, received false` (mock-state bleed / test-ordering pollution). BOTH pass deterministically when that file is run in ISOLATION, and a quiet re-run settled back to exactly the 8 environmental failures. LESSON: do NOT launch the baseline `npm test` while hunters are still running — machine load induces timeout + mock-pollution flakes that masquerade as real regressions (the `productTourPending` assertion looked exactly like a landed logic regression). Always confirm any non-Chrome/non-Shopify-env failure by re-running that ONE file in isolation before treating it as a bug.**
- No fixer subagents dispatched (no landed bug to fix) → NO fix PR opened (Phase 4: "if NO bug fixed, do not open a PR"). Empty `fix/nightly-2026-07-22` branch was created then deleted; repo left on main.
- **CI billing outage almost certainly STILL present** (GH Actions down since 2026-07-07 on a billing failure, maintainer-confirmed on #489) — did not need to inspect #642's runs; its red GH-Actions checks are infra noise. Gate locally + trust Vercel.
- **GitHub MCP:** `list_pull_requests {owner,repo,state:open,perPage:30}` returned 2 open PRs inline (no overflow): **#642** (this window's companion) + **#608** (keyuchen campaign sales-profit feature — UNCHANGED since last night's documented-CLEAN full review, updated 2026-07-21; did NOT re-comment). `pull_request_read {method:get_diff}` returns the full unified diff inline (used to judge #642 by-diff). `add_issue_comment` → `issue_number` (a PR number works). `gh` CLI still absent.

## 2026-07-21

### Triage — window = base `c49d446` → `7260303` (last night's tip → now); 14 merged PRs #603–#616; 1 landed bug fixed → PR #618. Fanned out 3 read-only hunters + 1 open-PR reviewer.
Window clusters: **Radar/SEO/GSC subsystem (BIG new feature, #616 `feat/radar`, John Duncan)** — `app/lib/seo/{gsc,search-console}.server.ts` (GSC OAuth + encrypted cred store + daily Search Analytics pull), `app/lib/radar/{detect.server,types}.ts` (traffic-drop / conversion-gap / stale-home / AEO / rankings-slip detectors), `app/routes/cron.seo-rankings.tsx` (daily drain), `dashboard.auth.gsc*.tsx`, `dashboard.api.search.tsx`, `Search.tsx`, migrations `radar_core`/`seo_rankings_summary_rpc`/`seo_settings_gsc_last_pulled`; **storefront custom-redesign mode (#615, keyuchen)** — `storefront-ai/redesign*.ts` + `design-guidance*.ts` + `command/intent/undo/release` + migration `storefront_atomic_custom_redesign`; **designer craft-doctrine (#612/#613)** — `designer/doctrine.ts` + `engine.server.ts` (prompt text); **storefront-recipes product-card simplify/restore (#604–#611, Eric Chen)** — recipe `bundle.ts` + `registry.ts` + `render.tsx` + `storefront-hydrator.tsx` (bulk of diff = `.webp` baseline noise). Hunters: (A) radar/seo/gsc, (B) redesign+designer, (C) recipes+runtime+sidebar. **1 bug found (hunter A) → PR #618** (branch `fix/nightly-2026-07-21`, commit `341cb08`, draft). B and C both CLEAN. **No companion "Mezoh" nightly-review PR this window** (last night's #602 merged f674b70; #597 merged c35b953). Only open PR = #608 (feature, reviewed clean).

### Bug fixed — Radar `detectTrafficDrops` scores a truncated top-path as a 100% traffic drop (merchant-facing WRONG NUMBER, #616)
`app/lib/radar/detect.server.ts` `detectTrafficDrops` (from `8e0753b`). The daily rollup RPC (`supabase/migrations/20260720130000_radar_core.sql:138` `where rn <= 20`) keeps only the **20 highest-view paths/day**, so `TrafficDay.topPaths` has ≤20 entries. The detector picks the 10 paths with the highest 7-day baseline total, then reads the last day's count via `last.topPaths.find(p=>p.path===path)?.views ?? 0`. A baseline-top-10 page (avg ≥30/day) merely **outranked below position 20** on the most recent day — but still nonzero (e.g. 25 views) → `.find` undefined → `?? 0` → scored as a total wipeout → merchant gets a `section_refresh` Radar move claiming "lost 100% of its visits / got 0 yesterday / down 100%" with a factually wrong number + a spurious apply-time generation brief. Reachable on busy stores (20+ paths out-trafficking a top page on a given day) — exactly where Radar matters. Fix (commit `341cb08`): only trust "absent == 0 views" when the last day's list is NOT truncated (`length < 20` → every real path present); when saturated (≥20) and absent, the true count is unknown → skip the candidate rather than fabricate a 100% drop. Added `export const TRAFFIC_ROLLUP_TOP_PATHS = 20` beside the traffic thresholds; regression test in `detect.signals.test.ts` asserts saturated-absent → no false drop AND genuine-zero-on-non-saturated-day → still fires.
- **Lesson (NEW family — truncated-list-as-zero): whenever a detector/analytic reads a per-item value out of a TOP-N-capped list via `list.find(x)?.value ?? 0`, "absent" conflates "genuinely zero" with "present but ranked > N". Defaulting to 0 fabricates a wrong extreme (here a 100% drop). Guard: only treat absence as zero when the list is NOT saturated (`length < N`); otherwise the true value is unknown — skip or bound it. On any new radar/analytics reader, grep for `.find(...)?.<field> ?? 0` (or `[...]?? 0`) drawing from a `LIMIT N` / `rn <= N`-capped source.**

### False positives cleared / verified CLEAN this run (do NOT re-flag)
- **Radar/SEO/GSC (#616) — the other 13 files CLEAN.** OAuth state-cookie CSRF (`__Host-` + Secure/Path=/SameSite=Lax, compared + cleared via `expireCookieHeader`, shopId from session not body); AES-256-GCM enc/dec symmetric (IV embedded); `requireSameOrigin` guards the `gsc_disconnect` write; daily drain idempotent (upsert on `(shop_id,query,page_url,captured_date)` unique constraint) with a correctly-advancing nulls-first cron cursor + per-shop failure isolation; **NO `.range()`/paged reads exist in this subsystem** (so the #1 tiebreak family doesn't apply here); `Search.tsx` imports only the client `search-client` module; sc-domain matching is dot-boundary. Cosmetic-only (did NOT flag): 14-vs-15-calendar-day labeling in `read_radar_ranking_series`, cur(8)/prev(7) day-count asymmetry in `read_seo_rankings_summary` — neither skips/double-counts rows.
- **Storefront custom-redesign (#615) + designer doctrine (#612/#613) — CLEAN.** `edit_storefront_draft` = proper CAS (conditional `where draft_version_id is not distinct from p_expected...` + `for update` + `40001` on miss); `restore_storefront_custom_version` takes `lock_storefront_design_shop` + re-selects `for update` on the full row; hashes round-trip; shopId from session; `replaceFont` replacement is a fixed non-`$` string now carrying `(?<![\w-])…(?![\w-])` boundaries; `var()` inliner uses a function replacer capped at 32 passes (these are the merged #602-era fixes, NOT regressions); 3-way redesign dispatch is if/else-if/else (no exhaustive-switch gap); asset keys allowlist-validated. `Store.tsx` `previewSrc` undefined for `page:${string}` is deliberate (covered by `store-logic.test.ts`).
- **Storefront-recipes product-card simplify (#604–#611) + runtime + sidebar (#614) — CLEAN.** Interactive-tag ↔ decorative-checkout guard-list coverage intact (`storefront-compiler/validate.ts:250` + `html.ts:257` already enumerate `button|details|summary|input|select` — the 2026-07-20 #602 `select` fix IS merged); no `ALLOWED_TAGS` list modified this window. Per-card quick-buy identity resolved from each host's own `authorityKey` (`product:<id>`) stamped per repeat iteration at COMPILE time — NO global `/g` variant replace (the recurring "looped control stamped with one global value" family is absent). `void fetcher(...).then(refresh)` sites in `storefront-hydrator.tsx` NOW carry `.catch(()=>{})` (2026-07-20 #602 fix #4 merged). `render.tsx`/`storefront-hydrator.tsx` reference `.server` only via `import type` (erased). Registry `VERSIONED_ASSET_MANIFESTS_BY_TEMPLATE_ID[id][templateVersion-1]` — every bumped version == its manifest-array length (no drift).
- **Open PR #608 (keyuchen, "dashboard/campaigns: add sales profit reporting", base main, NON-draft, still open) — CLEAN, no comment posted.** Money/analytics feature (campaign Sales-vs-Regular classification + account-wide Orders/Revenue/Profit/True-ROAS over 7/30/90d). Metrics from a single `rpc("campaign_performance")` whose SQL ends `order by c.name, c.id` (stable unique tiebreak — NO `.range()` paged read, so the #1 family doesn't apply). All cents; fee = `round(captured*0.029)+greatest(capture_count,1)*30`; profit = `revenue−cogs−carrier−fee−spend` each coalesced+counted once; True ROAS = `revenue/nullif(spend,0)` (div-0 guarded); CTEs grouped per `(campaign_id,order_id)` = no join fan-out inflating COGS/fees. Auth: shopId from session, RPC enforces `current_shop_id()` (42501 on mismatch), PATCH via requireSameOrigin+requireDashboardSession + `.eq("id").eq("shop_id",...)` + rejects extra body keys. Migration idempotent (`create or replace`, `add column if not exists`, guarded `to_regclass`), backfill scoped `classification_source='detected'`, indexes `(shop_id,day)`/`(shop_id,order_id)` match joins. Date window `[start 00:00 local, (end+1) 00:00 local)` (Pacific/Kiritimati boundary tested). `profit_cents` always-a-number with a `cost_complete` flag is INTENDED (Python test asserts `841` w/ `cost_complete False`). Not a nightly companion → no comment.

### Gate / environment
- Baseline clean tip (== main `7260303`, fix branch before edits): `npm ci` 0 · typecheck 0 · **`npm test` = 8 failed | 7124 passed | 12 skipped**. The 8 failures are ALL the environmental "No local Chrome found"/root-sandbox puppeteer failures — now **6** in `storefront-validation/browser.server.test.ts` (UP from 5; #615 added a buyer-actions Chromium proof), **1** `storefront-command/command.server.test.ts`, **1** `storefront-recipes/diagnostic-deck/bundle.test.ts`. Treat gate GREEN iff ONLY these + passing ≥ baseline. (IMPORTANT: capture `npm test` WITHOUT `| tail` and enumerate all N failures — memory warns tail can hide a real non-Chrome failure. This run confirmed all 8 are Chrome via the full "Failed Tests 8" block.)
- Fixer tree (`341cb08`): setup 0 · typecheck 0 · lint 0 (14 pre-existing warnings in untouched files; touched radar files warning-clean) · build 0 (verify-client-bundle **"Verified 2841 client files"** — NOTE: big jump from ~750 last week; the client-file count metric rose sharply this window, likely the custom-redesign + recipe assets — not a defect, just a moved baseline) · test 8 failed (all Chrome) | **7125** passed (+1 regression test) | 12 skipped.
- **CI runner-infra billing outage STILL present on #618 — SAME signature, did NOT re-kick/comment.** Node gate (88550712519) 05:27:06→05:27:09 = **3s**; Python engine tests (88550712473) same 3s, `failure` on a PURE-TS diff (impossible from code). Vercel Preview Comments = success, deploy = Building. Identical to every night since 2026-07-07 (maintainer confirmed on #489: GH Actions down on a billing failure). Gate locally + trust Vercel. #618 auto-subscribed — CI-failure + vercel[bot]-Building webhooks are expected infra noise.
- **GitHub MCP:** only 1 open PR this run (#608) — `list_pull_requests {owner,repo,state:open,perPage:30}` returned it inline (no overflow). `search_pull_requests` for merged-in-window OVERFLOWED the tool-result cap (61KB, single line) → auto-saved to file; but merged PRs map 1:1 to the window's merge commits from `git log`, so parsing it was UNNECESSARY (use git log for merge→PR mapping). `gh` CLI still absent. `pull_request_read {method:get_check_runs}` gives the CI billing-outage signature directly (compare `started_at`/`completed_at`). `create_pull_request` returns `{id,url}` with the PR number in the url path (#618).

## 2026-07-20

### Triage — window = 5 non-merge commits on `main` through `c49d446` (`57c9048`,`1635464`,`a4d0277`,`607dbb1`,`b54915e`); merged PRs #598/#599/#600/#601. Companion #602 already fixed ALL genuine bugs → posted ONE cross-check comment, did NOT open a duplicate fix PR.
The window: `1635464` dashboard/Autopilot decision-impact KPI small multiples (#599); `b54915e` hexagon-C brand mark unified into shared `CalderynHexMark` across every surface (#601); `a4d0277`+`607dbb1`+`57c9048` a large coordinated storefront-recipe refactor — versioned per-template asset manifests, contentHash-addressed `.webp`, curated-font expansion, "render approved source verbatim". Bulk of the diff is binary font/baseline `.webp` assets (noise). **Companion PR #602 (Mezoh, "Nightly review fixes — 2026-07-20", branch `nightly-review/2026-07-20`, base==main `c49d446`, open/non-draft, gate-green) reviewed this EXACT window and fixed 5 genuine bugs.** I independently re-triaged all 5 commits and verified each #602 fix against `main`:
- **#602 fix 1** `<select>` bypasses decorative-checkout interactive-control guard — GENUINE/live. `html.ts:29` ALLOWED_TAGS gained `select`/`option`/`label`/`input` (via `607dbb1`) but the two guards `html.ts:257` + `validate.ts:250` still enumerate only `button`/`details`/`summary`/`input` → a `<select>` compiles+validates+passes the inert-control proof (which skips `data-cd-native-control`). Fix = add `select` to both. (`label`/`option` correctly NOT guarded — non-interactive/select-scoped.)
- **#602 fix 2** unbounded `var()` inlining loop `factory.ts:~133` (`for(let prior="";prior!==value;)` over `sourceVariables`, no cycle cap) — GENUINE hang at module load on cyclic/self-growing first-party `--x`. Authoring footgun, not exploit. Bound at 32 passes.
- **#602 fix 3** boundary-less font-family regex `factory.ts:~121` `(["']?)FAMILY\1` — GENUINE substring over-match on suffixed ids (e.g. `Manrope`→`Manropes`). Add `(?<![\w-])`/`(?![\w-])`.
- **#602 fix 4** `void fetcher(...).then(refresh)` w/o `.catch` `storefront-hydrator.tsx:141,149` — GENUINE unhandled-rejection on network failure.
- **#602 fix 5** UTF-8 BOM `ef bb bf` at head of `storefront-command/types.ts` — GENUINE (confirmed via `od -tx1`).
Independent baseline on clean `main` `c49d446`: `npm ci` 0 · `tsc --noEmit` 0 · `storefront-compiler` 103/103 pass → all 5 are latent runtime/edge-case defects (none break compile/tests, matching #602's own gate). **No additional landed bug found, no false positive in #602's list.**

### DECISION — did NOT open a duplicate fix PR (applied the USUAL "don't duplicate a genuine companion" rule; this was NOT the #597-style exception).
Last night (#597↔#596) we DID open our own fix PR, but that was an explicit DEPARTURE justified by #596 being 3/5 STALE (already-on-main) → uncertain clean merge. **#602 is different: it targets THIS window's own brand-new code (`607dbb1` etc.), all 5 fixes genuine, no staleness/conflict risk, gate-green.** So the departure rationale doesn't apply → duplicating would be pure noise/review burden. Action taken: ONE cross-check comment on #602 corroborating all 5 as genuine+live, reporting the independent green baseline, and confirming no additional bug / no false positive; recommended merging #602. **If #602 is still open next night, do NOT re-triage its 5 items as new bugs — they are this window's landed defects, already fixed on its branch.**

### False positives cleared / verified CLEAN this run (do NOT re-flag)
- **`1635464` Autopilot KPI (#599) is CLEAN + correctly wired.** `AutopilotKpis` IS rendered (`Autopilot.tsx:485`); the new deps all exist — `app.executeAction` returns `{ok, receipt}`, `app.refreshLiveEngine()` (`context.ts:203`, `DashboardApp.tsx:730`), `app.liveEngine?.moneyProtectedWeekCents`, `client.rejectProposal` returns `receipt.{before,after}`; all screen test mocks already return `executeAction:()=>({ok,receipt:null})` + `refreshLiveEngine:()=>{}`. #602's note "on reject, Revenue-at-risk still subtracts impact" (`pendingAfter=reaction?max(0,pendingBefore-impact)`) is arguable UX semantics, NOT a confirmable logic bug — leave it.
- **`b54915e` brand-mark unification is CLEAN.** `oauth.authorize.tsx` + `orders-preview.tsx` (and 12 other surfaces) are pure SVG→`<CalderynHexMark>` component swaps; `CalderynHexMark` is now a single even-odd path in one `currentColor`-default color. No route/loader/auth/data behavior touched.
- **`a4d0277` `release-resolution.server.ts` contentHash rewrite is CLEAN.** `recipeDerivedStaticAssets` now resolves `version.assets` (was `recipe.bundle.assets`) and serves `/storefront-recipes/<id>/<contentHash>.webp`; the `registered.get(key)!` non-null assertion is SAFE — `staticEntries` only ever contains keys where `expected = registered.get(entry.key)` was truthy in the loop just above.

### Gate / environment
- Clean-`main` `c49d446` baseline: `npm ci` exit 0 · `tsc --noEmit` exit 0 · `storefront-compiler` 103/103 pass. (Did not run full `npm test` this run — window was fully covered by gate-green #602 and no fix branch was opened; the known "No local Chrome found" puppeteer failures from prior nights still apply if a full run is needed — `browser.server.test.ts` ×5, `command.server.test.ts` ×1, `diagnostic-deck/bundle.test.ts` ×1.)
- **Recurring pattern reinforced (ALLOWED_TAGS ↔ guard-list drift):** whenever a commit ADDS a tag to `storefront-compiler`'s `ALLOWED_TAGS`, grep the SAME file + `validate.ts` for the decorative-checkout interactive-control blocklist (`button|details|summary|input`) — a newly-allowed interactive control (`select` this window) routinely misses the parallel guard. Same "add-to-one-list-not-the-sibling" family as the `.catch`-guard-sibling-await miss (2026-07-19 bug #2).
- **Recurring pattern (BOM on freshly-touched module head):** a UTF-8 BOM keeps appearing at the head of the first `import` of files edited this refactor era (`storefront-command/types.ts` this night; `storefront-bundle/build.server.ts` 2026-07-17). tsc/esbuild/Vite strip a leading BOM so no runtime effect, but it violates clean-source hygiene — `od -An -tx1 -N3 <file>` == `ef bb bf` to detect.
- **GitHub MCP:** `list_pull_requests` requires `owner`+`repo` (bare call errors). `add_issue_comment` → `issue_number` (snake). `gh` CLI still absent — GitHub MCP only. Open PRs this run: #602 (new companion, this window), #597 (our 2026-07-19 draft, still open), #596 (2026-07-19 companion, still open) — #597/#596 were fully reviewed 2026-07-19, did NOT re-triage.

## 2026-07-19

### Triage — SMALL window: 11 non-merge commits `40754f8`→`c565727` (10 to triage; `40754f8` is our own #590); maintainer prod-log fixes + dashboard polish + a #589 revert; ALL 10 CLEAN. 2 landed bugs fixed came from OLDER merged code → PR #597
Small, tractable window — read all diffs directly, no fan-out needed. **Our prior nightly backlog is now FULLY MERGED**: git log shows Merge #590/#589/#557/#556/#525/#524/#489 all landed on `main`; only ONE open PR remains (#596 companion). The 10 window commits: storefront prod-log fixes (7dc2376 unpublished shops resolve to null not a thrown 503; 6414d90 invalid_store_intent → actionable 422; fccbdfb ship jetbrains-mono-latin.woff2), designer font hardening (c565727 `scrubDesignerCss` rewrites uncurated `/storefront-fonts/<id>` refs to a shipped substitute + engine prompt enumerates allowed ids; jetbrains-mono joins DESIGNER_FONT_IDS), 273dff3 storefront full-bleed for designer pages (new `rendersOwnDocument` shell-bypass gate), 9cd0bcb revert of #589's `hasExactKeys` tightening (renamed `hasNoUnknownKeys`), dashboard polish (ee0fcb9 compact-rail tooltips/hover-expand, 2a528c2 dark palette + iframe fade, abfb397 plain-language product copy + tour ending, c3437ce test stub for #589's requireSameOrigin). **All 10 window commits CLEAN.** The 2 bugs fixed this night were surfaced by reviewing companion #596 and live in OLDER merged designer/P&L code (see below) → PR #597 (branch fix/nightly-2026-07-19, commits c0cebaa + f16c58d, draft).

### Bug #1 fixed — operating-P&L dumps the ENTIRE opex onto the last product when there's no revenue to weight against (MONEY/analytics, #596's fix #5)
`app/lib/analytics/operating-pnl.ts` `allocateOperatingExpenses` (from the merged QuickBooks P&L work #549/#550). The per-product ternary checked `index === products.length - 1` **BEFORE** the `revenue === 0` guard: `index===last ? operatingExpensesCents - allocated : (revenue===0 ? 0 : proportional)`. When total weighting `revenue === 0` (a refund-only period that still carries non-zero opex), every non-last product got 0 and the LAST product got `operatingExpensesCents - allocated` = the WHOLE opex → wildly negative `netOperatingProfitCents` for one arbitrary product, 0 for the rest. Fix (commit c0cebaa): reorder so the guard wins for all — `revenue===0 ? 0 : (last ? remainder : proportional)`; +regression test asserting `[0,0]`. **NOTE: LEARNINGS 2026-07-17 marked this fn "remainder-to-last … correct" = a documented MISS (the zero-revenue edge wasn't considered).**
- **Lesson (NEW analytics family, distinct from the `.range()/.order` tiebreak family): a "remainder-to-last" allocator whose last-index branch runs BEFORE its zero-basis guard dumps the whole total onto one row when the weighting denominator is 0. Whenever you see `index===last ? total-allocated : (denom===0 ? 0 : proportional)`, the last-index short-circuit SKIPS the denom guard — the guard must be evaluated first.**

### Bug #2 fixed — unguarded `storeDataFor` read 500s the public designer storefront (resilience, #596's fix #2)
`app/lib/designer/serve.server.ts` `resolveDesignerPublicPage` (from the merged designer feature #549–#554). The sibling reads `getStoreSettings` (:117) and `loadPublication` (:123) are each `.catch(()=>null)` **explicitly** ("A read hiccup must never take the storefront down — fall through"), but `await storeDataFor(shopId, context)` at :130 was UNGUARDED. `storeDataFor` awaits getStoreSettings/loadDesignerAssets/catalog reads without catching (only its internal `applyAssetOverrides` is `.catch`ed) → a transient rejection propagates out of the loader as an unhandled 500 on the public designer storefront. Fix (commit f16c58d): wrap the await in the same `.catch → null`; the existing `if(!data) return null` then falls through to the runtime renderer. `storeDataFor` itself unchanged.
- **Lesson: when a route explicitly `.catch`-guards SOME upstream reads with a "never take the site down" comment, grep the SAME function for sibling `await`s that lack the guard — a later-added read routinely misses the pattern.**

### Companion PR #596 (Mezoh, "Nightly review fixes — 2026-07-19", base==our main d3108fc) — 2 GENUINE (#2,#5) + 3 ALREADY-ON-MAIN (#1,#3,#4). Posted ONE cross-check comment.
**CRITICAL judge-by-DIFF-not-body finding: #596 reviewed a PRE-#556 tree.** Its fixes #1 (sold-out availability), #3 (add-to-cart extra-classes token match), #4 (image/logo `src` escapeHtml) are ALL ALREADY on current main — serve.server.ts:61-69,146 (via the 2026-07-18 add-to-cart work) and render.server.ts:93,108 (via merged #556) → #596 will CONFLICT on serve/render when rebased. Only its #2 (storeDataFor guard) and #5 (operating-P&L zero-revenue) are genuine live bugs — the exact two we fixed in #597. Its "found but not fixed" #1 ("listing-page add-to-cart binds first product's variant") is ALSO stale — main already stamps per-product variant ids at render time (render.server.ts:124). Posted ONE comment: confirmed #2/#5 genuine, flagged #1/#3/#4 as already-shipped/will-conflict, recommended rebase-and-drop-3 or take #597.
- **DECISION RATIONALE for opening #597 even though #596 covers the same 2 bugs (a DEPARTURE from the usual "don't duplicate a genuine companion" rule): #596 is 3/5 stale → uncertain to merge cleanly, and bug #1 is a money/analytics correctness defect that shouldn't be gambled on a conflict-laden PR. #597 fixes exactly those 2, applies cleanly to the current tip, and the comment transparently discloses the overlap. If next night #596 is still open, do NOT re-triage its 5 items as new bugs — 3 are already on main, 2 are #597.**

### False positives cleared / verified CLEAN this run (do NOT re-flag)
- **All 10 window commits CLEAN.** Pins worth keeping: (a) 273dff3 `rendersOwnDocument` shell-bypass — `record.runtime === 1` without the old `"runtime" in data` check is behavior-identical (missing prop → `undefined !== 1`); designer gate is strict `=== true` (`designer:"true"` correctly excluded, tested). (b) c565727 `enforceCuratedFontFiles` — greedy `[a-z0-9-]+` capture correctly resolves multi-hyphen ids `source-serif-4`/`ibm-plex-mono` (backtracks to allow `-latin.woff2`); `substituteFontId` maps `mono|code`→ibm-plex-mono, `serif|slab|garamond|playfair|georgia`→source-serif-4, else inter; curated ids pass through. (c) 9cd0bcb `hasExactKeys`→`hasNoUnknownKeys` revert is CORRECT — #589's exact-length check rejected prompt commands that omit optional context/attachments keys (a real regression, 3 red types.test.ts). (d) ee0fcb9 sidebar tooltip delegated-event `useEffect` keyed to `[sidebarCompact]`, torn down + tip cleared on expand; UI-only, no data/auth/money surface. (e) 6414d90/7dc2376/fccbdfb are the maintainer's own prod-log fixes — all correct.
- **UTF-8 BOM reappears on `app/lib/storefront-command/types.ts:1`** (introduced by 9cd0bcb's diff, `-import`→`+﻿import`). Same class as the build.server.ts BOM noted 2026-07-17 — tsc/esbuild/Vite strip a leading BOM, no runtime/build effect. Hygiene nit, do NOT "fix" unattended.

### Gate / environment — SAME 7 environmental Chrome failures baseline; CI billing outage STILL present
- **Gate on branch tip (both fixers, first pass): setup 0 · typecheck 0 · lint 0 (14 pre-existing warnings in untouched files, up 1 from 13) · build 0 (verify-client-bundle "Verified 750 client files", up from 749) · npm test 6899 passed | 7 failed | 12 skipped.** The 7 failures are the SAME environmental "No local Chrome found"/root-sandbox ones documented since 2026-07-15, in exactly 3 files: storefront-validation/browser.server.test.ts (5), storefront-command/command.server.test.ts (1), storefront-recipes/diagnostic-deck/bundle.test.ts (1). Gate treated GREEN iff only-those-7 + passing ≥ 6899. `npm ci` exit 0 (once by orchestrator, reused by both sequential fixers).
- **CI runner-infra billing outage STILL present on #597 — SAME signature, did NOT re-kick/comment.** Node gate (88160178017) + Python engine tests (88160178035) BOTH `failure` on f16c58d; `get_check_run` output summary/text/title ALL "" (runner never executed); Python co-failed on a PURE-TS diff (operating-pnl + serve.server — impossible from code). Identical to every night since 2026-07-07. Vercel preview = Building (deploys fine). #597 auto-subscribed; CI-failure + vercel[bot]-Building webhooks are expected infra noise. Gate locally + trust Vercel.
- **GitHub MCP gotcha (NEW):** `list_pull_requests` now REQUIRES `owner`+`repo` params — the bare `minimal_output:true` call form used prior nights errors `missing required parameter: owner`. Only ONE open PR this run (#596) so no tool-result overflow. `add_issue_comment` → `issue_number` (snake); `get_check_run` → `checkRunId` (camel); `create_pull_request` returns `{id,url}` with the PR number in the url path. `gh` CLI still absent — GitHub MCP only.

## 2026-07-18

### Triage — BIG window: 64 non-merge commits `c6a7605`→`0d62ca1`, designer/storefront-command chat-builder REWRITE dominated; 1 landed TEST regression fixed → PR #590
Large active window. Last night's `app/lib/designer/*` subsystem was largely REWRITTEN/cut over this window into a chat-driven "hidden design" store builder consolidated onto ONE command route (`dashboard.api.store.tsx` action; 0de55f1 cut-over, c7bd78b orchestrate, 8c4c979 terminal CAS, 8a0d67a/e51140a harden boundaries, 9c6b648 classify+apply safe intents, a7042d8 constrain classifier). Other clusters: new **paged catalog/collection/search reads** (5a7c11a/9ccefb4/f677dec/965dbe1/b3f0d91/7f11421/de96eaf — keyset + `.range()`), **async product-image generation pipeline** (f521bad hero, 799d968 product photos, 295365e cron generate-missing, 100b6bf serve-ready, 84f808a bg-only, de96eaf bound), **Soft Chemistry recipe restore** (8bb0c06/937fd1f/fdc8970 — mostly webp baselines), **dashboard polish** (sidebar spacing, shipping static map, Locations, store build-UX, campaign icons, OnboardingTour). Fanned out 4 read-only hunters (designer serve/render · paged-read tiebreak · paid image-gen re-spend · command/intent/auth) — ALL CLEAN on their families. **1 landed TEST regression fixed → PR #590** (branch fix/nightly-2026-07-18, commit 40754f8, draft). NOTE local `main` branch ref was STALE at session start (detached HEAD 0d62ca1 was NEWER than the `main` ref 6f8c47d) — always `git fetch origin main` and base the fix branch on the true `origin/main` tip, not the local `main` ref.

### Bug fixed — impossible Request-equality assertion lands a test RED on main (test-gate regression, #589 miss)
`app/routes/__tests__/storefront.home-experiment.test.ts:95` (from `ee406c3` "merge hidden-design chat builder"). The "storefront home runtime cutover" test asserted `expect(trackStorefrontEventMock).toHaveBeenCalledWith(req(), "demo-shop", "page_view")` where `req()` builds a FRESH `new Request(url)` each call. **Vitest's `toHaveBeenCalledWith` compares args by value, and two identical `new Request(url)` instances are NOT deep-equal** (proved with an isolated probe: an assertion that they are UNEQUAL passed) → the assertion can NEVER pass; it fails with "expected to be called with arguments: […(3)]" + "Compared values have no visual difference" while the mock WAS called exactly once. The loader (`storefront._index.tsx:38` `trackStorefrontEvent(request, shopId, "page_view")`) is CORRECT — only the test expectation was wrong. With GH Actions down all week (billing), the red test went unnoticed on main. Fix (commit 40754f8, test-only, 1 line): assert `expect.any(Request)` — the idiom already used in the sibling `storefront.policies.test.tsx:61` — keeping the meaningful `shopId`/event-type checks.
- **Lesson: `toHaveBeenCalledWith(new Request(...), ...)` / `toEqual` on two distinct `Request` (or Response/Headers/URL) instances is a FALSE assertion — vitest treats them as unequal even when identical. The repo idiom is `expect.any(Request)`. When a NEW `*.test.ts` lands in a window, run the FULL `npm test` as a smoke check — the CI billing outage lets a red-on-landing test sit on main, and a companion PR that only reviews product diffs (like #589) won't catch a broken test. Same family as 2026-07-15 bug #3 (orphaned-component lint-gate regression landed red under broken CI).**

### Companion PR #589 (Mezoh, "Nightly review fixes — 2026-07-18", base==our main) — GENUINE, 7 fixes all correct (body says 6); MISSED the test regression → posted ONE cross-check comment
Reviewed #589's full diff: body says "Six" but the diff/Fixed list has **7**, ALL correct + minimal: (1) `browser.server.ts:726` proof handler `decodeURIComponent` OUTSIDE try/catch → URIError on a lone `%` in a fire-and-forget `void serveProofRequest` hangs the proof nav + unhandled rejection → decode-in-guard + respond 400; (2) `command.server.ts:594` one blank-title product (common via the maintained Shopify import) → `validProductCandidates` throws `StoreIntentClassificationError` → collapses to generic 500 for EVERY chat command for that store → filter empty-title before slicing candidates; (3) `dashboard.store.preview.tsx:131` missing `requireSameOrigin` on the preview-cart mutate/Set-Cookie action (defense-in-depth; cookies SameSite=Lax so not live-CSRF) → add it; (4) same file:170 `quantity` intent `Number()`→setQuantity throws → 500 on ordinary bad input → validate int 0–99 → 400; (5) `build.server.ts:116` `loadStorefrontRecipe` exhaustive switch w/o `default` → out-of-union persisted id → `Cannot read undefined` 500 instead of typed 409 → add default 409; (6) `types.ts:77` `hasExactKeys` didn't check `Object.keys().length===allowed.length` (latent foot-gun); (7) `ChatRail.tsx:82` auto-scroll keyed on `messages.length` misses in-place `replaceMessage` streaming updates → key on `messages`. All 7 target real landed bugs on main, no new bugs, disjoint from our #590 file. **Judge #589 by the DIFF — thorough + correct; do NOT re-triage its 7 fixes as new bugs next night if it merges.** It MISSED only the home-experiment test regression → posted one concise cross-check comment (precedent: #556 2026-07-17, #488 2026-07-15). Its own "found but not fixed" list (DesignerDock.tsx:249 same `messages.length` scroll blind spot, `flag:"wx"` baseline, unchunked product-id `IN`, `completeText:any`, `onAttachFiles` stale-state) is correctly reasoned lower-confidence — do not chase unattended.

### False positives cleared / verified CLEAN this run (do NOT re-flag)
- **Paged-read `.order(id)` tiebreak (THE #1 recurring family) — ALL CLEAN this window (notable — maintainer has internalized the rule).** Every new `.range()`-paged read carries a stable UNIQUE order: `catalog.owned.server.ts` child reads end in `id` PK or composite PKs (`variant_dim`/`product_media`/`product_option` `.order(...).order("id")`, `product_collection` `.order("product_id").order("collection_id")` = full PK); `context.server.ts` collection-COUNT read keyed on the `(product_id,collection_id)` PK pair (summed → still correct); `asset.server.ts applyAssetOverrides` `store_asset` keyed on `(shop_id,product_id,source)` unique — table has NO `id` col, so `created_at desc` in the MIDDLE is fine because the shop-scoped `(product_id,source)` already uniquely orders; `listProductPage`/`searchStorefront` are keyset (`title,id` cursor).
- **Designer add-to-cart variant stamping — FIXED by maintainer, verified.** Last night's bug (every product card adds product #1) is repaired: `render.server.ts:118-124` stamps each card's button with THAT product's own `data-variant-id` via a FUNCTION replacer at render time per-iteration; `serve.server.ts:145-148` global stamp now guarded by negative lookahead `(?![^>]*\bdata-(?:variant-id|designer-sold-out))` so it only fills the single un-stamped PDP buy box. Commit 1def8fa widened the class-token match (`\bdesigner-add-to-cart\b` inside a broader class list) correctly. Cart-drawer envelope (`{ok,data:{cart}}` via storefrontOk, read as `d.data.cart`) coherent end-to-end (47ddf45).
- **Paid image-gen re-spend (money) — CLEAN.** New cron `generate-missing.server.ts` (295365e) gates spend on the `claim_missing_storefront_images` RPC whose `candidates` CTE excludes products with a ready `store_asset` and only claims `pending`/`failed` → a generated product is never re-claimed/re-paid (gate table == write table). Flag-gated: `generateGeminiImages` throws on `!geminiImageGenerationEnabled()` BEFORE `meter.reserve` → flag-off = zero spend. Lease-claim idempotent (`for update skip locked` + finalize guarded on `status='pending' AND lease_expires_at=claim`). Ledger still fails closed (started-before-call, complete(failed) counts, release only when !providerStarted). Designer product-photos/hero (799d968/f521bad) gate on `applyAssetOverrides`/`designer_assets` (same write layer). TWO BELOW-BAR notes left for maintainer (act only if they recur): (a) lease TTL 5min == cron cadence `*/5` = zero-margin → a >5min batch could theoretically double-pay (harden lease to ~15min); (b) cron runs full seed+claim even when flag OFF = pointless DB write churn (no money) — a `if(!geminiImageGenerationEnabled()) return` short-circuit at the top of `generateMissingProductImages` avoids it.
- **Consolidated chat-command route auth — CLEAN.** `dashboard.api.store.tsx` action enforces `requireSameOrigin`→`requireDashboardSession`→POST→JSON content-type→rate-limit(session.shopId); `shopId=session.shopId`, `actorId=session.userId`; `parseStoreCommand`/`types.ts` has no tenant field; version ids UUID-validated. Terminal CAS = Postgres RPC conditional updates (`p_expected_draft_version_id`, atomic in DB); undo fully shop-scoped + CAS-guarded; edit-reconcile-on-timeout confirms `draft_version_id===resultVersionId` before returning success. NO new unauthenticated top-level route this window (only route file added is a test). Intent classifier schema-constrained (`STORE_INTENT_VALUE_SCHEMA`), compound requests rejected (`requestedOperations.length>1`), routing uses whole-token phrase spans (`findPhraseSpans` — the "inter"-in-"winter" substring class is structurally impossible). Build-failure logging (3353521) logs THEN emits error event AND rethrows — surfaced, not swallowed.
- **Designer serve/render escaping/sentinels/marker-leak — CLEAN.** Sentinel strip runs AFTER value substitution (value regex `[a-zA-Z0-9_.-]` can't match `#`/`/`); edit-marker leak guard coherent (CRLF normalize + absolute output guard engine.server.ts:439); edits.ts uses a function replacer for model text. Below-bar (UNCHANGED, do not re-flag): `serve.server.ts:147` string-replace embeds `data.contextVariantId` un-escaped (variant ids are internal, `$`-free → safe but inconsistent with the sibling function replacer); coupon-widget `widgets.ts:94` string-replace over escaped-but-not-`$`-escaped model attrs (previously noted below-bar).

### Gate / environment — NEW GOTCHA: 7 environmental "No local Chrome" failures on baseline main (root-container puppeteer sandbox)
- **Baseline clean main (`0d62ca1`) is NOT 0-failed this window: `npm test` = 8 failed | 6887 passed | 12 skipped.** Of the 8, **7 are `Error: No local Chrome found`** — the storefront browser-PROOF tests (`storefront-recipes/diagnostic-deck/bundle.test.ts` [NEW this window, ee406c3], `storefront-validation/browser.server.test.ts`, and some in `storefront-command/command.server.test.ts`) call `proveStorefrontBundle`→puppeteer-core→`launchChromium` (`app/lib/browser/chromium.server.ts`), which throws with no local Chrome. Setting `PUPPETEER_EXECUTABLE_PATH=/opt/pw-browsers/chromium` (the pre-installed Playwright Chromium) lets it FIND Chrome but it then dies **"Running as root without --no-sandbox is not supported"** — the harness doesn't pass `--no-sandbox`, so these CANNOT be made green in this root CI container (CI runs them on Vercel/AWS with `@sparticuz/chromium`). These 7 are ENVIRONMENTAL: **treat the gate GREEN iff the ONLY test failures are these exact "No local Chrome"/root-sandbox ones AND passing ≥ baseline.** Do NOT let a fixer ralph-loop chase them.
- The **8th** failure was the real one (fixed → PR #590). Post-fix gate on `40754f8`: setup 0 · typecheck 0 · lint 0 (13 pre-existing warnings in untouched files) · build 0 (verify-client-bundle **749** client files clean) · **test 6888 passed | 7 failed (all No-local-Chrome) | 12 skipped**. `npm test` exit code is 1 SOLELY due to the 7 environmental Chrome failures.
- `npm ci` exit 0 (installed ONCE by orchestrator). Full suite ~110s. IMPORTANT: capture `npm test` to a file WITHOUT `| tail` — piping through tail truncated the failure list and hid that 1 of the 8 was a real assertion (not Chrome); had to re-run.
- **CI runner-infra billing outage STILL present on #590 — SAME signature, did NOT re-kick/comment.** Node gate (88048280050) + Python engine tests (88048280121) BOTH `failure` on `40754f8`; Python `get_check_run` output summary/text/title ALL `""` (runner never executed) on a PURE one-line TS test diff (a Python failure is impossible from that code). Identical to every night since 2026-07-07 (maintainer confirmed #489: GH Actions down all week on a billing failure). Vercel preview = Building (deploys fine). Gate locally + trust Vercel. PR #590 auto-subscribed on open — the CI-failure + vercel[bot]-Building webhooks are expected infra noise.
- `gh` CLI STILL absent — GitHub MCP only. `list_pull_requests state:open minimal_output:true perPage:30` returned 8 open PRs but OVERFLOWED the tool-result cap (~56KB) → auto-saved to a file; parse with python `json.load`. Open PRs: #589 (new companion), #557/#525/#489 (our own prior drafts, still open/unmerged), #556/#524 (old companions), #395 (storegen), #267 (RR7). `get_check_run` param is `checkRunId` (camel); `add_issue_comment` param is `issue_number` (snake).
