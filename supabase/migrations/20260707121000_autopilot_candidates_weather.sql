-- Add weather_demand to the autopilot candidate allow-list so the
-- inventory-relocation branch in app/lib/actions/autopilot.server.ts can act on
-- weather nudges (campaign_id / sku_id resolved from entity_ref, transfer plan
-- from alert_context.evidence). The rest of the view body is verbatim from
-- 20260627120000_autopilot_candidates_resume.sql — only the detector_id IN list
-- gains 'weather_demand'.
