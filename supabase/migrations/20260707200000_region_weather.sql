...
-- inline on every buyer request. Global, non-shop-scoped, non-sensitive, so it
-- carries a read-for-all RLS policy; writes are service-role only (the cron).
...
create policy region_weather_public_read on public.region_weather for select using (true);
