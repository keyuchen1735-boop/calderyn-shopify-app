...
--
-- First normalize any rows still parked on a now-removed step name to a valid one,
-- THEN replace the constraint — adding the constraint before the data is migrated
-- would fail on those rows. Merchants mid-provider-connect (or on creative_mapping)
-- map to the consolidated 'connect' step; shopify/guardrails/consent/complete are
-- unchanged and already valid. NULL is permitted (NULL IN (...) is NULL, not false).
--
