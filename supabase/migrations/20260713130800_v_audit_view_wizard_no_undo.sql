-- create_campaign_wizard is audited-but-NOT-undoable: undo.server.ts has no
-- branch for it (there is no "un-create" that could be trusted — the wizard's
-- run-state machine owns cleanup), so its audit rows must never advertise an
-- undo affordance. Same treatment 20260703030300 gave issue_refund; the
-- update/duplicate campaign kinds ARE undoable and stay eligible.
--
-- Split from 20260713130759 (which registers the enum value) because a new
-- enum value cannot be referenced in the transaction that adds it.
--
-- GOTCHA (create or replace view is append-only): every column is copied
-- verbatim, in order, from the prior definition (20260703030300) — only the
-- undo_eligible predicate changes.
...
select
  ...
  -- and create_campaign_wizard has no undo executor, so both are excluded here
  -- regardless of window.
  aa.outcome = 'succeeded'::action_outcome
    and aa.action_kind <> 'issue_refund'::action_kind
    and aa.action_kind <> 'create_campaign_wizard'::action_kind
