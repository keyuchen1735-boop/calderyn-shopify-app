...
-- is bleeding on this SKU. Only orders with a REAL ship cost contribute — an
-- unresolved cost is never treated as $0 (that would fake a profit and hide
-- leakage). NULL when the SKU has no in-window order with a real ship cost,
-- so the column renders "no data" rather than a misleading $0.
--
-- Fallback-$0 exclusion (the only change vs 20260616140000): the ship-cost
-- resolver writes a FALLBACK placeholder of $0 (ship_cost_source = 'fallback',
-- ship_cost_confidence = 'low', ship_cost_cents = 0) for every order that has no
-- real cost input. A fallback-$0 is non-null but is NOT a real cost — counting
-- it computes "collected − $0 = positive", showing FAKE profit instead of honest
-- no-data. So an order contributes only when it has a REAL cost: a manual
-- override (ship_cost_manual_cents), OR a resolved ship_cost_cents whose source
-- is not the 'fallback' placeholder. SKUs whose in-window orders are ALL fallback
-- → ship_pnl_cents NULL → the column renders "—" (honest no-data) instead of a
-- fake positive.
--
