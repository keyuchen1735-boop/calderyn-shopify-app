// Calderyn DashV2 — <image-slot> user-fillable image placeholder.
// Ported from image-slot.js. A native custom element (Web Component), not a
// React component — drop <image-slot id="..."> into JSX/HTML. The dropped image
// persists across reloads via a .image-slots.state.json sidecar (read via fetch,
...
type StoredSlot = string | SlotView;
...
let slots: Record<string, StoredSlot> = {};
  ...
  loadP = fetch(STATE_FILE)
    ...
    .then((j) => {
      ...
      if (j && typeof j === "object") {
        const merged: Record<string, StoredSlot> = Object.assign({}, j, slots);
        // A framing-only write that raced ahead of hydration must not
        // drop a user image that's only on disk — inherit u from the
        // sidecar for any in-memory entry that lacks one.
        for (const k in slots) {
          const m = merged[k];
          if (m && typeof m !== "string" && !m.u && j[k]) {
            m.u = typeof j[k] === "string" ? j[k] : j[k].u;
          }
        }
...
function getSlot(id: string): SlotView | null {
  const v = slots[id];
  if (!v) return null;
  return typeof v === "string" ? { u: v, s: 1, x: 0, y: 0 } : v;
}

function setSlot(id: string, val: StoredSlot | null): void {
  if (!id) return;
  if (val) {
    slots[id] = val;
...
export class ImageSlot extends HTMLElement {
  ...
  private _spill!: HTMLElement;
  private _ghost!: HTMLImageElement;
  private _err: HTMLElement | null;
  private _input!: HTMLInputElement;
  private _depth: number;
  private _gen: number;
  private _view: SlotView;
  private _subFn: () => void;
  private _local: StoredSlot | null = null;
  ...
  constructor() {
    ...
    root.addEventListener("click", (e) => {
      ...
      const act = target && target.getAttribute && target.getAttribute("data-act");
      if (act === "replace") {
    ...
    this._spill.addEventListener("pointerdown", (e) => {
      ...
      const corner = targetEl && targetEl.getAttribute && targetEl.getAttribute("data-c");
  ...
  _exitReframe(commit: boolean): void {
    if (!this.hasAttribute("data-reframe")) return;
  ...
  async _ingest(file: File): Promise<void> {
    this._setError(null);
    ...
    try {
      ...
      this._exitReframe(false);
      const val: SlotView = { u: url, s: 1, x: 0, y: 0 };
      setSlot(this.id || "", val);
  ...
  _geom(): { iw: number; ih: number; fw: number; fh: number; base: number } | null {
  ...
  _commitView(): void {
    const v: SlotView = { s: this._view.s, x: this._view.x, y: this._view.y };
  ...
  _render(): void {
    ...
    let stored = this.id ? getSlot(this.id) : asView(this._local);
...
// Normalize the session-local value (id-less slots) into a SlotView, matching
// getSlot's behavior for stored values.
function asView(v: StoredSlot | null): SlotView | null {
  if (!v) return null;
  return typeof v === "string" ? { u: v, s: 1, x: 0, y: 0 } : v;
}
