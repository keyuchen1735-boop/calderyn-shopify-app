...
import { foldAndDock, groupForActionKind } from "../hero/hero-motion";
...
function PendingRow({ proposal, app, onResolved }: { proposal: QueueProposalVM; app: DashboardCtx; onResolved: (alertId: string) => void }) {
  ...
  const group = groupForActionKind(proposal.action_kind);
  ...
  const onApprove = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const { ok } = await app.executeAction(alert, proposal.action_kind as ActionKind);
      if (!ok) return; // error toast already fired in executeAction
      setState("approved");
      app.refreshCalibration();
      // Optimistic calibration bump + ensure the hero reasoning panel is open so
      // the destination group row exists before the chip docks.
  ...
  return (
    <div className="cd-trace-row" data-tag="REVIEW" data-pending="1" style={{ cursor: "default", flexDirection: "column", alignItems: "stretch" }}>
  ...
  return (
    <div className="cd-card" style={{ padding: 0 }}>
      ...
      <div className="cd-rows cd-log-scroll" style={{ paddingBottom: 6, maxHeight: 362, overflowY: "auto" }}>
        {livePending.map((p) => (
          <PendingRow key={p.alertId} proposal={p} app={app} onResolved={markResolved} />
        ))}
        {trace.map((t) => (
