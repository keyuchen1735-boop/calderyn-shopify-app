// Composable MapLibre wrapper for the dashboard: <Map> owns the map instance
// and theme-matched basemap style; markers, popups, tooltips, routes, arcs and
// cluster layers compose as children. Styling uses the mapcn-* classes in
// dashboard.css (cd-* variables), not a CSS framework.
import MapLibreGL, { type PopupOptions, type MarkerOptions } from "maplibre-gl";
// tsconfig's `types` allowlist excludes ambient @types/geojson, so import it.
...
// Lucide imported directly (not via the dashboard CDIcon registry): this is a
// standalone ui/ primitive that must not depend on dashboard modules.
...
// The dashboard applies its theme as .cd-dark on .cd-root (not on <html>), so
// theme detection watches that element first, then falls back to the system.
function themeRoot(): Element {
  return document.querySelector(".cd-root") ?? document.documentElement;
}

function getDocumentTheme(): Theme | null {
  if (typeof document === "undefined") return null;
  const root = themeRoot();
  if (root.classList.contains("cd-dark") || root.classList.contains("dark")) return "dark";
  if (root.classList.contains("cd-root") || root.classList.contains("light")) return "light";
  return null;
}
...
function useResolvedTheme(themeProp?: "light" | "dark"): Theme {
  ...
  useEffect(() => {
    ...
    // Re-check after mount: the dashboard defaults to dark for SSR parity and
    // applies a persisted light choice post-mount.
    const docTheme = getDocumentTheme();
    if (docTheme) setDetectedTheme(docTheme);

    // Watch for theme-root class changes (night-mode toggle).
    const observer = new MutationObserver(() => {
      const next = getDocumentTheme();
      if (next) setDetectedTheme(next);
    });
...
type MapProps = {
  ...
  className?: string;
  /**
   * Theme for the map. If not provided, automatically follows the dashboard's
   * night-mode class (with system preference as fallback).
   */
  theme?: Theme;
  ...
  return (
    <div className="mapcn-loader">
      <div className="flex" style={{ gap: 4 }}>
        ...
        <span className="mapcn-loader-dot" style={{ animationDelay: "150ms" }} />
        <span className="mapcn-loader-dot" style={{ animationDelay: "300ms" }} />
      </div>
    </div>
  );
}
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    // If the basemap style/tiles never load (offline, blocked by an
    // extension), drop the loader overlay instead of spinning forever; layer
    // children stay gated on the style via isStyleLoaded.
    const errorHandler = () => setIsLoaded(true);
  ...
  return (
    <MapContext.Provider value={contextValue}>
      <div ref={containerRef} className={cn("mapcn", className)}>
...
function MarkerContent({ children, className }: MarkerContentProps) {
  ...
  return createPortal(
    <div className={cn("mapcn-marker", className)}>{children || <DefaultMarkerIcon />}</div>,
    marker.getElement(),
  );
}

function DefaultMarkerIcon() {
  return <div className="mapcn-pin" />;
}

function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} aria-label="Close popup" className="mapcn-close">
      <X size={14} />
    </button>
  );
}
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(
    <div className={cn("mapcn-tooltip", className)}>{children}</div>,
    container,
  );
}
...
function MarkerLabel({ children, className, position = "top" }: MarkerLabelProps) {
  return (
    <div
      className={cn(
        "mapcn-label",
        position === "top" ? "mapcn-label-top" : "mapcn-label-bottom",
        className,
      )}
    >
      {children}
...
type MapControlsProps = {
  ...
  /** Callback when locating fails (no geolocation support, denial, or timeout) */
  onLocateError?: () => void;
};

const positionClasses = {
  "top-left": "mapcn-controls-tl",
  "top-right": "mapcn-controls-tr",
  "bottom-left": "mapcn-controls-bl",
  "bottom-right": "mapcn-controls-br",
};

function ControlGroup({ children }: { children: React.ReactNode }) {
  return <div className="mapcn-ctlgroup">{children}</div>;
}
...
function MapControls({
  ...
  const handleFullscreen = useCallback(() => {
    ...
    // requestFullscreen is absent on iPhone Safari; a denied request rejects.
  ...
  return (
    <div className={cn("mapcn-controls", positionClasses[position], className)}>
      {showZoom && (
        <ControlGroup>
      ...
      {showLocate && (
        <ControlGroup>
          ...
          >
            {waitingForLocation ? (
              <Loader2 size={16} className="mapcn-spin" />
            ) : (
              <Locate size={16} />
            )}
          </ControlButton>
...
function CompassButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <ControlButton onClick={onClick} label="Reset bearing to north">
      {/* Hand-drawn on purpose: the two-tone needle rotates in 3D with the
          map bearing/pitch, which no stock icon glyph supports. */}
      <svg
        ref={compassRef}
        viewBox="0 0 24 24"
        width={20}
      ...
      >
        <path d="M12 2L16 12H12V2Z" fill="var(--red)" />
        <path d="M12 2L8 12H12V2Z" fill="var(--red-bg)" />
        <path d="M12 22L16 12H12V22Z" fill="var(--text-3)" />
        <path d="M12 22L8 12H12V22Z" fill="var(--hairline-strong)" />
      </svg>
    </ControlButton>
  );
}
