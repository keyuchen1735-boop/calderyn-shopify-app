...
/**
 * The full Live Engine surface — autopilot features, the live pipeline, the
 * action trace + inspector, and the calibration/predictions rail. Stateless
 * apart from the local selection + freshness ticker, so it can mount on either
 * the home route or the legacy engine route. The feature toggle posts to the
 * route that mounts it (no action url), so each host wires its own action.
 */
