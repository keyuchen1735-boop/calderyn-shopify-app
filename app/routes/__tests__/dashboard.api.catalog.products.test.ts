...
vi.mock("~/lib/catalog/catalog.server", () => ({ listProducts, createProduct, getProduct, updateProduct: vi.fn(), setProductStatus: vi.fn() }));
vi.mock("~/lib/catalog/validate", () => ({ validateProductInput: (b: unknown) => ({ ok: true, value: b }) }));
const signMediaPaths = vi.fn().mockResolvedValue(new Map([["shop1/p1/x.png", "https://signed/x"]]));
vi.mock("~/lib/catalog/sign-media.server", () => ({ signMediaPaths, signMediaPath: vi.fn() }));

beforeEach(() => { listProducts.mockClear(); createProduct.mockClear(); getProduct.mockReset(); signMediaPaths.mockClear(); });
...
describe("catalog products route", () => {
  ...
  it("GET :id reshapes the stored detail into the editor view-model", async () => {
    getProduct.mockResolvedValue({
      id: "p1", title: "Tee", status: "active", vendor: "Acme", category: "Shirts", description: null,
      tags: ["new"],
      options: [{ id: "o1", name: "Size", values: [{ id: "ov1", value: "S" }, { id: "ov2", value: "M" }] }],
      variants: [
        { id: "var-m", sku: "TEE-M", title: "M", retailPriceCents: 1999, unitCostCents: null, inventoryTracked: null, inventoryOnHand: 4, optionValueIds: ["ov2"] },
      ],
      media: [{ id: "m1", storagePath: "shop1/p1/x.png", alt: null, position: 0, isPrimary: true }],
      collectionIds: ["c1"], updatedAt: "t",
    });
    const { loader } = await import("../dashboard.api.catalog.products.$id");
    const res = (await loader({ request: new Request("https://app.x/dashboard/api/catalog/products/p1"), params: { id: "p1" } } as never)) as Response;
    expect(res.status).toBe(200);
  ...
  it("GET :id 404s when the product is not the shop's", async () => {
    ...
    await expect(
      loader({ request: new Request("https://app.x/dashboard/api/catalog/products/nope"), params: { id: "nope" } } as never),
