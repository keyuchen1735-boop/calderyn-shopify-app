...
describe("/pilot/unsubscribe", () => {
  it("400s an invalid token on GET", async () => {
    ...
    const res = await loader(routeArgs({ request: new Request("https://app.test/pilot/unsubscribe?token=bad"), params: {}, context: {} }));
  ...
  it("records the opt-out on POST with a valid token", async () => {
    ...
    const res = await action(routeArgs({ request: new Request(`https://app.test/pilot/unsubscribe?token=${token}`, { method: "POST" }), params: {}, context: {} }));
