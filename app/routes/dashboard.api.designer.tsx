...
import { getStoreSettings, setComposerEnabled } from "~/lib/storefront/settings.server";
import {
  designerBuildState,
  designerFirstBuild,
  designerTurn,
...
/** Studio boot: is the sparkle on, is a build complete, and the saved chat. */
export async function loader({ request }: LoaderFunctionArgs) {
  const session = await requireDashboardSession(request);
  const settings = await getStoreSettings(session.shopId);
  if (!settings.composerEnabled) {
    return json({ enabled: false, ready: false, unbuiltRoutes: [], chat: [] });
  }
  const [state, chat] = await Promise.all([
    designerBuildState(session.shopId),
    loadDesignerChat(session.shopId),
  ]);
  return json({ enabled: true, ready: state.hasDocuments && state.unbuiltRoutes.length === 0, unbuiltRoutes: state.unbuiltRoutes, chat });
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // Studio management actions (sparkle toggle, publish) share this route so
  // the whole designer surface stays out of the classic builder's API.
  if (body.action === "set-enabled") {
    if (typeof body.enabled !== "boolean") return jsonError(422, "invalid_body");
    await setComposerEnabled(session.shopId, body.enabled);
    return json({ enabled: body.enabled });
  }
  if (body.action === "publish") {
    const settingsNow = await getStoreSettings(session.shopId);
    if (!settingsNow.composerEnabled) {
