...
import { priceCart, removeCartLine } from "~/lib/order/cart.server";
...
export async function action({ request }: ActionFunctionArgs): Promise<Response> {
  ...
  if (!(await allowStorefrontRequest(request, shopId, "cart-remove"))) return storefrontError(429, "rate_limited");
  const body = await parseStrictObject(request, ["lineId"]);
