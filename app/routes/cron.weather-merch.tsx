...
// Refresh the global region_weather table (4 rows) that the public storefront
// reads to merchandise weather-relevant products. One batched Open-Meteo fetch
// for all region centroids, then an upsert — so buyer requests never pay the
// external forecast call. Fail-closed: on a forecast error we leave the last-good
// rows in place rather than blanking them.
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return json({
    updated: rows.length,
    regions: rows.map((r) => ({ region: r.region, condition: r.condition })),
  });
};
