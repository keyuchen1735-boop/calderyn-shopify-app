...
// GET /dashboard/login → 302 into Shopify OAuth. With a valid ?shop= (the
// embedded app's "Open dashboard" deep link) we 302 to that shop's authorize
// URL and pin the state cookie to `nonce:shop`; without it we start Shopify's
// admin authorize, which signs the merchant in and routes the grant to their
// store — no store domain is collected on our side — and the cookie pins
// `nonce:*`. ?error= renders the friendly failure state (oauth_failed,
// app_not_installed) instead of raw JSON.
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (rawShop !== null) {
    // An explicit store domain (the embedded app's "Open dashboard" deep link):
    // normalize a bare handle / pasted URL, then validate strictly.
  ...
  // state and fails with oauth_failed. Route OAuth init through the canonical
  // host once so the cookie and the callback share an origin. The apex proxy
  // presents every request to this server as the app origin, so the marker — not
  // the host comparison alone — is what breaks the self-redirect loop.
  ...
  // No ?shop= (direct visit, /login's "Continue with Shopify", connect
  // redirect): start Shopify's admin authorize — it signs the merchant in and
  // routes the grant to their store, so we never ask for the domain. The state
  // cookie then pins `*` and the callback binds the shop from the HMAC-verified
  // callback params. A known shop pins to that exact shop.
  ...
  // through the round-trip, URL-encoded so its query can't collide with the
  // cookie's `:` separators. Shop-less initiation pins the `*` sentinel.
...
export default function DashboardLoginPage() {
  ...
  // Retry re-enters OAuth with everything the failed attempt carried — the shop
  // (when known) and the post-login destination (connector consent).
