...
export const meta: MetaFunction = () => [{ title: "Our story" }];
export const headers: HeadersFunction = ({ loaderHeaders }) => loaderHeaders;

export async function loader({ request }: LoaderFunctionArgs) {
  const shopId = await resolveStorefrontShop(request);
  const runtime1 = await resolveRuntime1Route({ shopId, request, route: { kind: "story" } });
  if (!runtime1) throw new Response("Storefront is temporarily unavailable.", { status: 503 });
  const nonce = randomBytes(18).toString("base64url");
  const responseHeaders = storefrontCacheHeaders({ routeId: "story", personalized: false, shopId });
...
export default function StorefrontStory() {
  ...
  return <>{renderStorefrontSurface({ bundle: loaded.bundle, routeId: loaded.routeId, data: loaded.data, nonce: loaded.nonce, mode: "public", visualLayerPlacement: loaded.visualLayerPlacement })}<StorefrontHydrator bundle={loaded.bundle} routeId={loaded.routeId} data={loaded.data} mode="public" /></>;
}
