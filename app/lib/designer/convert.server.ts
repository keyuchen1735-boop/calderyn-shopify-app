...
import { loadStorefrontRecipe } from "../storefront-bundle/build.server";
...
export async function convertTemplateToDocuments(templateId: StoreTemplateId): Promise<{ documents: DesignerDocument[]; baseCss: string }> {
  const recipe = await loadStorefrontRecipe(templateId, getStoreTemplate(templateId).activeVersion);
