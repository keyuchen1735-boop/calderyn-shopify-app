...
- Conversion is the job: above the fold the store's promise plus ONE dominant call to action; price and availability visible on every product card; the product page's buy area (title, price, stock, add to cart) readable without scrolling; the cart's checkout button is the heaviest element on that page; somewhere on home a quiet trust row (shipping, returns, guarantee); every section ends somewhere useful, no dead ends.
...
function firstBuildInstruction(input: {
  ...
  const base = input.mode === "template"
    ? `First build, page "${input.route}": this store was just attached to the "${input.templateId}" template. Rework this page so it belongs to THIS merchant: their copy, their accent, their category names. Keep the template's structural bones where they serve the page, and fully style anything the template left plain.`
