...
describe("ownedCatalog.listProducts", () => {
  ...
  it("fails closed on conflicting singleton facts and derives facets through shop-scoped active rows", async () => {
    tableRows = {
      product_dim: [{ id: "p1", handle: "chair", title: "Chair", description: "" }],
      variant_dim: [], product_media: [], product_collection: [], product_option: [],
      product_fact: [
        { id: "f1", product_id: "p1", kind: "dimension.width", number_value: 80, unit: "cm", text_value: null, url_value: null },
        { id: "f2", product_id: "p1", kind: "dimension.width", number_value: 32, unit: "in", text_value: null, url_value: null },
        { id: "f3", product_id: "p1", kind: "material", text_value: "Walnut", number_value: null, unit: null, url_value: null },
      ],
    };
    const { ownedCatalog } = await import("../catalog.owned.server");
    const [product] = await ownedCatalog.listProducts("shop-1");
    expect(product.facts).toEqual([]);
    const facets = await ownedCatalog.listProductFactFacets?.("shop-1");
    expect(facets).toEqual({});
    expect(eqCalls.product_fact).toContainEqual(["shop_id", "shop-1"]);
    expect(eqCalls.product_fact).toContainEqual(["product_dim.status", "active"]);
  });
