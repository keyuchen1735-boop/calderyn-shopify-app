...
import { SHADER_SOURCE_CAP } from "../storebuilder/fx/shader";

const PROMPT_CODE_POINT_CAP = 4_000;
...
export function parseStoreCommand(value: unknown): ParseStoreCommandResult {
  ...
  if (input.kind === "prompt") {
    if (!hasExactKeys(input, ["kind", "prompt", "expectedDraftVersionId", "context", "attachments"])
      ...
      || Array.from(input.prompt).length > PROMPT_CODE_POINT_CAP
      ...
      || (input.attachments !== undefined
        && (!Array.isArray(input.attachments) || !input.attachments.every(isStoreAttachment)))) {
