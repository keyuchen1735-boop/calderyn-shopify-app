...
const store = vi.hoisted(() => {
  ...
  const rpc = vi.fn(async (): Promise<{ data: null; error: { message: string } | null }> => ({ data: null, error: null }));
  class Builder {
...
import { saleFallbackForOrder } from "../engine.server";
// eslint-disable-next-line import/first
import { projectLevelFact } from "~/lib/inventory/project-level-fact.server";

describe("saleFallbackForOrder", () => {
  beforeEach(() => {
    for (const k of Object.keys(store.db)) store.db[k].length = 0;
    store.rpc.mockClear();
    (projectLevelFact as unknown as ReturnType<typeof vi.fn>).mockClear();
  ...
  it("decrements only tracked variants via inventory_sale_fallback, keyed idempotently per (order, variant)", async () => {
    const res = await saleFallbackForOrder("shop-1", "o-1");
    expect(res).toEqual({ decremented: 2 });
    expect(store.rpc).toHaveBeenCalledTimes(1);
    expect(store.rpc).toHaveBeenCalledWith("inventory_sale_fallback", {
      p_shop_id: "shop-1", p_variant_id: "v-tracked", p_location_id: "loc-1",
      p_qty: 2, p_idempotency_key: "salefb:o-1:v-tracked",
    });
    expect(projectLevelFact).toHaveBeenCalledWith("shop-1", "v-tracked", "loc-1");
  });
  ...
  it("returns 0 and calls nothing when no line is tracked", async () => {
    ...
    const res = await saleFallbackForOrder("shop-1", "o-1");
    expect(res).toEqual({ decremented: 0 });
    expect(store.rpc).not.toHaveBeenCalled();
  });

  it("returns 0 and calls nothing when the order has no lines", async () => {
    store.db.order_line.length = 0;
    const res = await saleFallbackForOrder("shop-1", "o-2");
    expect(res).toEqual({ decremented: 0 });
    expect(store.rpc).not.toHaveBeenCalled();
  });

  it("throws when the RPC errors, so the caller's try/catch can log it loudly", async () => {
    store.rpc.mockResolvedValueOnce({ data: null, error: { message: "deadlock" } });
    await expect(saleFallbackForOrder("shop-1", "o-1")).rejects.toBeTruthy();
  });
});
