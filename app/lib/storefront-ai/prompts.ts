...
const CONCEPT_SOURCE_CONSTRAINTS = `Concept shell/home are static compiler-safe previews. The runtime renders leading shell chrome, then the active route, then one top-level shell footer under the same shell parent. shell.html must never contain main, a route outlet, or a content host; it may contain only leading announcement, header, and navigation chrome plus one top-level footer with store identity and policy links. Put all main content in home.html, and never reserve route height or width from shell. Use no buttons, trusted slots, states, bindings to state, or interaction actions. Neither shell.html nor home.html may contain the literal <slot tag or data-cd-slot. Root anchors use no route parameters and never target product or collection; route them only to home, search, cart, checkout, or account. IDs and references never cross between shell and home. Root data bindings are limited to store.name, store.logo, and cart.count. Product bindings appear only inside a featured.products repeat. Product anchors inside featured.products require data-cd-param-handle="product.handle". The repeat parent has only data-cd-repeat; set data-cd-repeat="featured.products" on it. Put data-cd-key="product.id" on a descendant and invent no helper data-cd attributes. Home must use a responsive desktop multi-column featured.products grid: create a static grid wrapper whose direct child is the repeated product card. Never put data-cd-repeat on the grid wrapper because that repeats the whole grid as stacked one-card rows. At a 1440px viewport, at least two repeated product cards must share the first row; never use a single-column desktop product list. Keep product cards compact with aspect-ratio or bounded pixel/rem sizing; never use viewport-relative heights or min-heights for product cards. The first 1440x800 and 390x844 overview frames must both contain a prominent store or product heading, a short commerce value proposition, and either a clear catalog anchor or visible product cards. Never leave the first viewport mostly blank, empty, or purely decorative; featured product content must begin within each overview frame. Never rotate or vertically orient navigation text or the store wordmark, and keep visible navigation text at least 14px. Every repeated product card must remain visually complete with a prominent product title of at least 16px and an authored CSS fallback surface when product.primaryImage or price is unavailable; do not rely on empty image or money bindings for card structure. globalCss must be empty; keep concept styles in shell.css and home.css. In shell.css and home.css, never select any data-cd-* attribute (including [data-cd-key]); style repeat descendants with authored classes. Every CSS selector stays away from trusted commerce hosts because concepts contain no slots.`;
...
export function judgePrompt(
  ...
  return `Score candidate ${candidate.candidate.concept.name} from 0-100 on every required dimension using four real Chromium screenshots ordered desktop overview, desktop catalog, mobile overview, then mobile catalog. Concept previews intentionally contain no buttons, trusted commerce slots, states, or interactions; score navigation anchors and static affordance hierarchy without requiring add-to-cart, filters, or other controls forbidden at this stage. For interactionClarity, treat absent forbidden controls as neutral and never deduct for missing filters, add-to-cart, hover states, or secondary actions. Do not penalize missing product imagery or prices when CONTEXT_DATA does not provide them; instead judge whether authored fallback surfaces and product titles remain deliberate and prominent. When CONTEXT_DATA has fewer than two products, treat unused catalog columns as neutral and do not call the grid broken or incomplete solely because only one real product can render. Still fail unstyled or broken layouts, weak product prominence, poor responsive adaptation, ecommerce ambiguity, or recipe convergence.
