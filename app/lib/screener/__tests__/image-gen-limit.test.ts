...
describe("decideImageGenLimit", () => {
  ...
  it("allows when both counts are under their limits", () => {
    expect(decideImageGenLimit({ shopCount: 5, globalCount: 100, ...limits })).toEqual({ ok: true });
  });

  it("blocks with scope 'shop' when the per-shop count has reached the limit", () => {
    expect(decideImageGenLimit({ shopCount: 20, globalCount: 100, ...limits })).toEqual({
      ok: false,
      scope: "shop",
      limit: 20,
    });
  ...
  it("blocks with scope 'global' when the global count has reached the limit", () => {
    expect(decideImageGenLimit({ shopCount: 5, globalCount: 300, ...limits })).toEqual({
      ok: false,
      scope: "global",
      limit: 300,
    });
  ...
  it("reports the shop limit first when both are exceeded", () => {
    expect(decideImageGenLimit({ shopCount: 25, globalCount: 400, ...limits })).toEqual({
      ok: false,
      scope: "shop",
      limit: 20,
    });
...
describe("imageGenLimits", () => {
  ...
  it("defaults to 20 per shop and 300 global", () => {
    expect(imageGenLimits()).toEqual({ perShopDaily: 20, globalDaily: 300 });
  });
