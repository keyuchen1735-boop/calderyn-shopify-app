...
async function updateIntegrationProgress(
  ...
  const payload = completed == null
    ? { destination_repair_attempted_at: attemptedAt }
    : {
        destination_repair_completed_at: completed ? attemptedAt : null,
        destination_repair_attempted_at: attemptedAt,
      };
...
export async function repairOrderDestinationsForIntegration(
  ...
  try {
    const { data, error } = await sb
      ...
      .or("customer_city.is.null,customer_region.is.null,customer_country.is.null")
    ...
    if (!candidates.length) {
      await updateIntegrationProgress(sb, integration, attemptedAt, true);
    ...
    for (const row of candidates) {
      ...
      if (columns.length) {
        let destinationUpdate = sb
          ...
          .is("destination_repair_checked_at", null);
        for (const column of columns) destinationUpdate = destinationUpdate.is(column, null);
        const { error: destinationError } = await destinationUpdate;
        if (destinationError) throw destinationError;
        updated += 1;
      }

      const { error: checkedError } = await sb
        .from("order_fact")
    ...
    await updateIntegrationProgress(sb, integration, attemptedAt, completed);
    return { scanned: candidates.length, updated, checked, completed };
  } catch (error) {
    try {
      await updateIntegrationProgress(sb, integration, attemptedAt, null);
    } catch (progressError) {
      console.error("[destination-repair] failed to record attempt", integration.shopId, progressError);
    }
    throw error;
  }
}
