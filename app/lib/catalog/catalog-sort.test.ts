...
describe("catalogSortToOrder", () => {
  ...
  it("maps status_asc / status_desc to status", () => {
    expect(catalogSortToOrder("status_asc")).toEqual({ column: "status", ascending: true });
    expect(catalogSortToOrder("status_desc")).toEqual({ column: "status", ascending: false });
  });
  it("exposes exactly the five sorts", () => {
    expect(CATALOG_SORTS.map((s) => s.value)).toEqual([
      "updated",
      "title_asc",
      "title_desc",
      "status_asc",
