...
/** The editor PUTs the complete product. Older active imports can predate the
 * shipping-dimension gate, so an unrelated edit must not force merchants to
 * repair shipping data first. The exception is deliberately derived from the
 * authoritative stored product: activation, variant-set changes, and any edit
 * to a shipping field still run the current completeness check. */
function shippingValidationRequired(
  status: ProductStatus,
  variants: VariantInput[],
  previous?: ProductDetail,
): boolean {
  if (!previous || previous.status !== "active" || status !== "active")
    return true;
  if (variants.length !== previous.variants.length) return true;

  const previousById = new Map(
    previous.variants.map((variant) => [variant.id, variant]),
  );
  for (const variant of variants) {
    if (!variant.id) return true;
    const stored = previousById.get(variant.id);
    if (!stored) return true;
    if (
      (variant.weightGrams ?? 0) !== (stored.weightGrams ?? 0) ||
      (variant.lengthMm ?? null) !== (stored.lengthMm ?? null) ||
      (variant.widthMm ?? null) !== (stored.widthMm ?? null) ||
      (variant.heightMm ?? null) !== (stored.heightMm ?? null) ||
      (variant.requiresShipping ?? true) !==
        (stored.requiresShipping ?? true) ||
      (variant.handlingDays ?? 0) !== (stored.handlingDays ?? 0) ||
      (variant.signatureRequired ?? false) !==
        (stored.signatureRequired ?? false) ||
      JSON.stringify(variant.restrictedCountries ?? []) !==
        JSON.stringify(stored.restrictedCountries ?? [])
    ) {
      return true;
    }
  }
...
export function validateProductInput(
  raw: unknown,
  context: { previous?: ProductDetail } = {},
  ...
  if (r.seo !== undefined && r.seo !== null) {
    if (typeof r.seo !== "object" || Array.isArray(r.seo))
    ...
    seo = {
      metaTitle: clampCodePoints(
        (typeof s.metaTitle === "string" ? s.metaTitle : "").trim(),
        SEO_TITLE_MAX,
      ),
      metaDescription: clampCodePoints(
  ...
  const status = STATUSES.includes(r.status as ProductStatus)
    ? (r.status as ProductStatus)
    : null;
  if (!status) return { ok: false, code: "invalid_status" };
  if (!Array.isArray(r.variants) || r.variants.length === 0)
    return { ok: false, code: "no_variants" };
  if (r.variants.length > MAX_VARIANTS)
    return { ok: false, code: "too_many_variants" };
  ...
  const variants = (r.variants as unknown[]).map((v) => {
    ...
    return {
      ...
      retailPriceCents: Number.isFinite(o.retailPriceCents)
        ? Number(o.retailPriceCents)
        : undefined,
      compareAtPriceCents: Number.isFinite(o.compareAtPriceCents)
        ? Number(o.compareAtPriceCents)
        : undefined,
      unitCostCents: Number.isFinite(o.unitCostCents)
        ? Number(o.unitCostCents)
        : undefined,
      inventoryPolicy:
        typeof o.inventoryPolicy === "string" ? o.inventoryPolicy : undefined,
      inventoryTracked:
        typeof o.inventoryTracked === "boolean"
          ? o.inventoryTracked
          : undefined,
      inventoryOnHand: Number.isFinite(o.inventoryOnHand)
        ? Math.max(0, Math.trunc(Number(o.inventoryOnHand)))
        : 0,
      optionValues: Array.isArray(o.optionValues)
        ? (o.optionValues as unknown[]).filter(
            (x): x is string => typeof x === "string",
          )
        : undefined,
      weightGrams: Number.isFinite(o.weightGrams)
        ? Number(o.weightGrams)
        : undefined,
      ...
      requiresShipping:
        typeof o.requiresShipping === "boolean"
          ? o.requiresShipping
          : undefined,
      handlingDays: Number.isFinite(o.handlingDays)
        ? Number(o.handlingDays)
        : undefined,
      signatureRequired:
        typeof o.signatureRequired === "boolean"
          ? o.signatureRequired
          : undefined,
      restrictedCountries: Array.isArray(o.restrictedCountries)
        ? (o.restrictedCountries as unknown[]).filter(
  ...
  const ISO2 = /^[A-Za-z]{2}$/;
  const enforceShipping = shippingValidationRequired(
    status,
    variants,
    context.previous,
  );
  for (const v of variants) {
    if (v.retailPriceCents != null && v.retailPriceCents < 0)
      return { ok: false, code: "negative_price" };
    if (v.compareAtPriceCents != null && v.compareAtPriceCents < 0)
      return { ok: false, code: "negative_compare_at" };
    if (v.unitCostCents != null && v.unitCostCents < 0)
      return { ok: false, code: "negative_cost" };
    ...
    if (
      status === "active" &&
      v.retailPriceCents != null &&
      v.retailPriceCents <= 0
    )
    ...
    if (v.restrictedCountries?.some((c) => !ISO2.test(c)))
    ...
    if (status === "active" && physical && enforceShipping) {
      if (
        !(v.weightGrams && v.weightGrams > 0) ||
        !(v.lengthMm && v.lengthMm > 0) ||
        !(v.widthMm && v.widthMm > 0) ||
        !(v.heightMm && v.heightMm > 0)
      ) {
  ...
  const options = Array.isArray(r.options)
    ? (r.options as unknown[])
        .map((o) => {
          const oo = (typeof o === "object" && o ? o : {}) as Record<
            string,
            unknown
          >;
          ...
          const values = [
            ...new Set(
              (Array.isArray(oo.values)
                ? (oo.values as unknown[]).map(String)
                : []
              )
                .map((v) => v.trim())
                .filter(Boolean),
  ...
  if (options && options.some((o) => o.values.length > MAX_VALUES_PER_OPTION))
    return { ok: false, code: "too_many_option_values" };
  ...
  return {
    ...
    value: {
      ...
      options,
      variants,
      collectionIds: Array.isArray(r.collectionIds)
        ? (r.collectionIds as unknown[]).filter(
            (c): c is string => typeof c === "string",
          )
        : undefined,
    },
