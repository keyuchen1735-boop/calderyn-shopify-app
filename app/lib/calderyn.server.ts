    ...
    integrations: {
      ...
      async disconnect(provider: string, _signal?: AbortSignal): Promise<void> {
        try {
          ...
          // (EasyPost/ShipBob = pasted key; ShipHero = OAuth token). Disconnect means the
          // merchant wants that credential gone too — so the cron's connect() finds no
          // credential and marks the shop skipped, not errored. (The ad OAuth providers
          // keep their bytea token row on shop_integrations; re-connect overwrites it.)
          // Surface a failure here rather than leaving an orphaned credential silently
          // behind (rule 12).
          const SHIP_COST_CRED_KINDS = new Set(["easypost_ship", "shipbob_ship", "shiphero_ship"]);
