...
const { apiGet, throwIfVersionSkew } = vi.hoisted(() => ({
  apiGet: vi.fn(),
...
describe("sendStoreCommand", () => {
  const commands: StoreCommand[] = [
    { kind: "prompt", prompt: "make it warmer", expectedDraftVersionId: VERSION },
    {
      ...
      attachments: [
        { kind: "design_reference", assetRef: "owned-asset" },
  ...
  it("checks deployment version skew before reading the command stream", async () => {
    const response = responseFromChunks([
      '{"stage":"ready","receipt":{"status":"unchanged","message":"No change"}}\n',
    ]);
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response));

    await sendStoreCommand(
      { kind: "prompt", prompt: "build", expectedDraftVersionId: null },
      () => undefined,
    );

    expect(throwIfVersionSkew).toHaveBeenCalledWith(response);
  });
