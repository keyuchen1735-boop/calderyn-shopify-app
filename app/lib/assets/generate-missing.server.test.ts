...
describe("store asset generation claim migration", () => {
  ...
  ).find((name) => name.endsWith("_store_asset_generation_claims.sql"));
  ...
  it("uses a canonical ordered Supabase migration version", () => {
    expect(migrationName).toMatch(/^\d{14}_store_asset_generation_claims\.sql$/);
    expect(migrationName! > "20260716180000_designer_documents.sql").toBe(true);
  });
