import sourceTemplate from "../../../../docs/superpowers/prototypes/storefront-recipes/soft-chemistry.html?raw";
import { defineRecipe, sourceTemplateCss, type RecipeConfig } from "../factory";
import { SOFT_CHEMISTRY_ASSETS } from "./assets";

const shell = {
  html: `<header class="head"><button class="quiet" data-cd-route="collection">Shop formulas</button><a class="brand" data-cd-route="home">Soft Chemistry</a><nav class="nav"><button class="quiet" data-cd-route="collection">Skin</button><button class="quiet" data-cd-route="collection">Body</button><button class="quiet" data-cd-route="search">Ingredients</button><button class="quiet" data-cd-route="home">Our standard</button></nav><div class="tools"><button class="quiet" data-cd-route="search">Search</button><button class="quiet" data-cd-route="cart">Bag (<span data-cd-text="cart.count"></span>)</button></div></header><aside class="visually-hidden" data-cd-slot="cartDrawer" data-cd-host-size="panel" data-cd-theme-tokens="milk ink yellow"></aside><footer class="visually-hidden" data-cd-policy-links></footer>`,
...
shell.css = sourceTemplateCss(sourceTemplate, shell.html, { display: "Cormorant Garamond", body: "Manrope" })
  + ".head{background:#281d2c}";

const home = {
  html: `<main class="view"><section class="hero"><img data-cd-asset="hero" alt="Glass skincare bottles in a luminous formulation studio"><div class="hero-copy"><span class="kicker">Formulation note 07 · barrier ritual</span><h1>Skin, in its softer state.</h1><p>High-touch formulas for low-noise routines. Explore the live collection by concern, texture, or place in ritual.</p><button class="cta" data-cd-route="collection">Enter the formulation studio <b>→</b></button></div><div class="orbital" aria-hidden="true"><span class="molecule m1">Beta<br>glucan</span><span class="molecule m2">Oat<br>lipids</span><span class="molecule m3">PHA<br>3%</span></div></section><section class="ritual"><div class="ritual-copy"><span class="kicker">Build a quiet ritual</span><h2>Three steps. Nothing extra.</h2><p>Begin with a concern and the studio assembles available products in their correct sequence. Every formula sheet is grounded in merchant-supplied ingredients and usage.</p><button class="cta" data-cd-route="collection">Begin with your skin</button></div><div class="formula-stack" data-cd-repeat="featured.products"><a class="sheet" data-cd-key="product.id" data-cd-route="product" data-cd-param-handle="product.handle"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><h3 data-cd-text="product.title"></h3><span class="visually-hidden" data-cd-text="product.description"></span><b><span data-cd-text="product.availability"></span> · <span data-cd-money="product.price"></span></b></a></div></section><p class="empty" data-cd-empty-state>No formulas.</p><section class="concerns"><button class="concern" data-cd-route="collection"><small>Concern 01</small><span>Dryness</span><b>8 formulas →</b></button><button class="concern" data-cd-route="collection"><small>Concern 02</small><span>Barrier</span><b>6 formulas →</b></button><button class="concern" data-cd-route="collection"><small>Concern 03</small><span>Texture</span><b>4 formulas →</b></button><button class="concern" data-cd-route="collection"><small>Concern 04</small><span>Scalp</span><b>5 formulas →</b></button></section><section class="products"><div class="section-head"><h2>The current formulas</h2><p>Live price, volume, stock, and variants from the merchant catalog. Select a sheet to read the complete dossier.</p></div><div class="rail" data-cd-repeat="featured.products"><article class="card" data-cd-key="product.id"><a data-cd-route="product" data-cd-param-handle="product.handle"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><div class="card-info"><h3 data-cd-text="product.title"></h3><span class="visually-hidden" data-cd-text="product.description"></span><b data-cd-money="product.price"></b></div></a></article></div></section></main>`,
