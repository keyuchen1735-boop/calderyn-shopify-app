...
const home = {
  html: `<main class="gilt-home"><section class="gilt-hero"><div><p class="gilt-kicker">Object ceremony / live collection</p><h1 class="gilt-title">A small thing, held forever.</h1><p>Objects chosen slowly, presented with their current price and availability.</p><a class="gilt-cta" data-cd-route="collection">Enter the collection</a><aside class="gilt-risk"><p>Review the merchant's current return and shipping terms.</p><nav data-cd-policy-links></nav></aside></div><div class="gilt-hero-visual"><img class="gilt-hero-image" data-cd-asset="hero" alt="Minimal gold objects arranged for gifting" width="1600" height="900"><div class="gilt-orbit" data-cd-repeat="featured.products"><article data-cd-key="product.id"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><span data-cd-text="product.title"></span><p data-cd-text="product.description"></p></article></div></div></section><section><p class="gilt-kicker">The current cabinet</p><div class="gilt-grid" data-cd-repeat="featured.products"><article class="gilt-card" data-cd-key="product.id"><a data-cd-route="product" data-cd-param-handle="product.handle"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><h2 data-cd-text="product.title"></h2><p data-cd-text="product.description"></p><p data-cd-money="product.price"></p><small data-cd-text="product.availability"></small></a></article></div></section><p class="gilt-empty" data-cd-empty-state>The cabinet is being prepared.</p></main>`,
  css: `${baseCss}.gilt-home{background:var(--cream);color:var(--black)}.gilt-hero{display:grid;grid-template-columns:.9fr 1.1fr;min-height:48rem}.gilt-hero>div:first-child{align-self:start;display:flex;flex-direction:column;justify-content:center;min-height:42rem;padding:clamp(2rem,6vw,6rem);position:sticky;top:4rem}.gilt-hero-visual{position:relative;min-height:48rem;overflow:hidden;background:var(--black)}.gilt-hero-image{width:100%;height:100%;object-fit:cover}.gilt-orbit{position:absolute;inset:auto 0 0;background:linear-gradient(transparent,var(--black));color:var(--cream);display:grid;grid-auto-flow:column;grid-auto-columns:min(80%,24rem);overflow-x:auto;scroll-snap-type:x mandatory}.gilt-orbit article{display:grid;grid-template-columns:7rem 1fr;gap:.5rem 1rem;min-height:11rem;padding:1rem;align-items:end;scroll-snap-align:start}.gilt-orbit article p{grid-column:2;margin:0}.gilt-orbit img{grid-row:1/3;filter:saturate(.75) contrast(1.05);height:8rem;object-fit:contain;width:7rem}.gilt-home>section:nth-child(2)>p{padding:3rem 2rem 1rem}@media(max-width:760px){.gilt-hero{grid-template-columns:1fr;min-height:auto}.gilt-hero>div:first-child{min-height:auto;position:static}.gilt-hero-visual{min-height:32rem}}@media(prefers-reduced-motion:reduce){.gilt-orbit{scroll-behavior:auto}.gilt-orbit img{transform:none}}`,
  requiredData: [], requiredCapabilities: [],
};
...
const product = {
  html: `<main class="gilt-object">
    ...
    <section class="gilt-grid" data-cd-repeat="related.products"><article class="gilt-card" data-cd-key="product.id"><a data-cd-route="product" data-cd-param-handle="product.handle"><h2 data-cd-text="product.title"></h2><p data-cd-text="product.description"></p><p data-cd-money="product.price"></p></a></article></section>
  </main>`,
...
export const GILT_RECIPE = compileRecipeConfig(GILT_RECIPE_CONFIG);
export const GILT_BUNDLE = GILT_RECIPE.bundle;
