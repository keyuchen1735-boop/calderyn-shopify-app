...
const boxBuilderFrames = Array.from({ length: 7 }, (_, count) => `<div class="larder-box-board"><div class="larder-box-slots">${Array.from({ length: 6 }, (_slot, index) => `<div class="larder-box-slot${index < count ? " is-selected" : ""}">${index + 1}</div>`).join("")}</div><strong>${count} of 6 selected</strong></div>`).join("");
...
export const LARDER_RECIPE_CONFIG = {
  ...
  surfaces: {
    ...
    home: {
      ...
      source: route(`
        <main class="larder-page larder-home">
          ...
          <section id="larder-box" class="larder-box">
            ...
            <div class="larder-box-workbench"><div id="larder-box-states" class="larder-box-states" aria-label="Six-slot pantry box" data-cd-state-id="box-count" data-cd-state-type="index" data-cd-state-initial="0" data-cd-state-min="0" data-cd-state-max="6" data-cd-bind-state="box-count" data-cd-bind-property="activeIndex">${boxBuilderFrames}</div><div class="larder-box-controls"><button value="remove" data-cd-on="click" data-cd-action="state.decrement" data-cd-state="box-count">Remove place</button><button value="add" data-cd-on="click" data-cd-action="state.increment" data-cd-state="box-count">Fill next place</button><a data-cd-route="cart">Review basket</a></div></div>
          </section>
      ...
      `, `${sharedCss}
        ...
        .larder-box{display:grid;grid-template-columns:.8fr 1.2fr;gap:clamp(2rem,7vw,7rem);padding:clamp(3rem,7vw,7rem);background:var(--tomato);color:var(--cream)}.larder-box button,.larder-box a{min-height:44px;border:1px solid currentColor;background:transparent;color:inherit;padding:.7rem;text-decoration:none}.larder-box-board{display:grid;gap:1rem}.larder-box-slots{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.larder-box-slot{display:grid;aspect-ratio:1;place-items:center;border:2px dashed var(--cream);border-radius:50%;font-family:var(--font-display);font-size:2rem}.larder-box-slot.is-selected{background:var(--olive);border-style:solid}.larder-box-controls{display:flex;flex-wrap:wrap;gap:.75rem;margin-top:1rem}
        @media(max-width:720px){.larder-shelf-tabs{grid-template-columns:repeat(2,1fr)}.larder-product-shelf{grid-template-columns:1fr}.larder-box{grid-template-columns:1fr}.larder-box-slots{grid-template-columns:repeat(2,1fr)}}@media(prefers-reduced-motion:reduce){.larder-film,.larder-hero-copy,.larder-stock{animation:none;transition:none}}
      `),
    },
    collections: {
    ...
    checkout: {
      ...
      source: {
        html: `<header class="larder-checkout"><span>Final pantry check</span><h1 data-cd-text="store.name"></h1><p>Review live quantities and delivery before secure payment.</p></header><aside class="larder-checkout-note"><b>Packed from the live basket</b><span>Final totals shown by checkout</span><span>Merchant policies remain available</span></aside><footer data-cd-policy-links></footer>`,
