...
const heroVideo = videoFragment({ className: "ember-hero-film", posterAssetKey: "hero-poster", webmAssetKey: "hero-webm", mp4AssetKey: "hero-mp4" });
const storyVideo = videoFragment({ className: "ember-story-film", posterAssetKey: "hero-alt-poster", webmAssetKey: "hero-alt-webm", mp4AssetKey: "hero-alt-mp4" });
const detailVideo = videoFragment({ className: "ember-detail-film", posterAssetKey: "pdp-detail-poster", webmAssetKey: "pdp-detail-webm", mp4AssetKey: "pdp-detail-mp4" });
...
export const EMBER_RECIPE_CONFIG = {
  ...
  surfaces: {
    ...
    home: {
      ...
      source: route(`
        <main class="ember-page ember-home">
          <section class="ember-hero" aria-label="Sauce pouring across a black tasting counter">${heroVideo.html}<div class="ember-hero-copy" ${motionAttributes("reveal")}><span class="ember-kicker">Heat / taste / repeat</span><h1 class="ember-display">Find your<br>bright burn.</h1><p>Move through the merchant's current catalog by declared heat, flavor, price, and availability.</p><a class="ember-button" data-cd-route="collection">Taste the lineup</a></div><div class="ember-spectrum" aria-label="Heat spectrum"><span>Mellow</span><i></i><span>Wild</span></div></section>
          <section class="ember-flight" ${motionAttributes("reveal")}><header><span class="ember-kicker">Tasting flight / live catalog</span><h2 class="ember-display">Build a tasting flight.</h2><p>Open current products to choose real variants and add each bottle or snack through protected commerce controls.</p></header><div class="ember-flight-rail" data-cd-repeat="featured.products"><article data-cd-key="product.id"><a data-cd-route="product" data-cd-param-handle="product.handle"><img data-cd-src="product.primaryImage" data-cd-alt="product.title" width="720" height="900"><h3 data-cd-text="product.title"></h3></a><p data-cd-text="product.description"></p><div class="ember-flight-meta"><b data-cd-money="product.price"></b><span data-cd-text="product.availability"></span></div></article></div><p class="ember-empty" data-cd-empty-state>No products are ready for this tasting flight.</p></section>
          <aside class="ember-proof-strip" aria-label="Current catalog proof"><b>Raw stock signal</b><div class="ember-proof-list" data-cd-repeat="featured.products"><span data-cd-key="product.id"><strong data-cd-text="product.title"></strong><i data-cd-text="product.availability"></i></span></div></aside>
          <section id="ember-ugc" class="ember-ugc" aria-label="Customer tasting notes" data-cd-state-id="ugc-hidden" data-cd-state-type="boolean" data-cd-state-initial="true" data-cd-bind-state="ugc-hidden" data-cd-bind-property="hidden"></section>
        </main><section data-cd-repeat="featured.products"><div data-cd-key="product.id" data-cd-slot="quickViewCommerce" data-cd-product="product.id" data-cd-host-size="inline" data-cd-theme-tokens="char bone pepper"></div></section>
      `, `${sharedCss}
        .ember-hero{position:relative;display:grid;min-height:min(860px,90dvh);align-items:end;overflow:hidden;padding:clamp(1.5rem,7vw,7rem);background:radial-gradient(circle at 72% 45%,rgba(242,41,24,.45),transparent 8%),linear-gradient(115deg,#070606 46%,#35100c);isolation:isolate}.ember-hero:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(11,9,9,.94),transparent 68%);z-index:-1}.ember-hero-film{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:-2}.ember-hero-copy{max-width:48rem}.ember-hero h1{font-size:clamp(4.6rem,12vw,11rem);margin:.5rem 0}.ember-hero-copy p{max-width:34rem;font-size:1.15rem;line-height:1.5}.ember-spectrum{position:absolute;right:1.5rem;bottom:1.5rem;display:grid;grid-template-columns:auto minmax(120px,20vw) auto;gap:.75rem;align-items:center;text-transform:uppercase}.ember-spectrum i{height:10px;background:linear-gradient(90deg,var(--bone),var(--ember),var(--pepper))}.ember-flight{padding:clamp(3rem,7vw,7rem)}.ember-flight header{max-width:64rem}.ember-flight h2{font-size:clamp(3.5rem,8vw,8rem);margin:.5rem 0 2rem}.ember-flight-rail{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1px;background:var(--ash)}.ember-flight-rail article{display:grid;gap:.9rem;padding:1rem;background:var(--coal)}.ember-flight-rail img{width:100%;aspect-ratio:4/5;object-fit:cover}.ember-flight-rail a{color:inherit;text-decoration:none}.ember-flight-meta{display:flex;justify-content:space-between;gap:1rem}.ember-proof-strip{display:grid;grid-template-columns:180px 1fr;padding:1.25rem;background:var(--pepper);color:var(--bone);text-transform:uppercase}.ember-proof-list{display:grid}.ember-proof-strip span{display:grid;grid-template-columns:1fr auto;gap:1rem;padding:.55rem;border-bottom:1px solid rgba(255,244,223,.45)}.ember-proof-strip i{font-style:normal}.ember-ugc{display:none}
        ...
        @media(prefers-reduced-motion:reduce){.ember-hero-film,.ember-hero-copy,.ember-flight{animation:none;transition:none;scroll-behavior:auto}}
      `),
    },
    collections: {
    ...
    product: {
      ...
      source: route(`<main class="ember-page ember-product"><section class="ember-product-media"><img data-cd-src="product.primaryImage" data-cd-alt="product.title" width="1000" height="1200"><div data-cd-repeat="product.images"><img data-cd-key="product.primaryImage" data-cd-src="product.primaryImage" data-cd-alt="product.title" width="800" height="1000"></div><div class="ember-detail-fallback">${detailVideo.html}</div></section><article><span class="ember-kicker">Live bottle / current facts</span><h1 class="ember-display" data-cd-text="product.title"></h1><b data-cd-money="product.price"></b><strong data-cd-text="product.availability"></strong><p data-cd-text="product.description"></p><h2>Available options</h2><div class="ember-variants" data-cd-repeat="product.variants"><div class="ember-variant" data-cd-key="variant.id"><b data-cd-text="variant.title"></b><span data-cd-text="variant.availability"></span><strong data-cd-money="variant.price"></strong></div></div><nav data-cd-policy-links></nav></article></main><section><div data-cd-slot="variantPicker" data-cd-product="product.id" data-cd-host-size="block" data-cd-theme-tokens="char bone pepper"></div><div data-cd-slot="addToCart" data-cd-product="product.id" data-cd-host-size="block" data-cd-theme-tokens="char bone pepper"></div></section>`, `${sharedCss}.ember-product{display:grid;grid-template-columns:minmax(0,1.25fr) minmax(22rem,.75fr)}.ember-product-media{display:grid;gap:1px;background:var(--ash)}.ember-product-media img{width:100%;aspect-ratio:4/5;object-fit:cover}.ember-detail-fallback{min-height:300px;background:radial-gradient(circle at 50% 40%,var(--pepper),var(--char) 55%)}.ember-detail-film{width:100%;height:100%;min-height:300px;object-fit:cover}.ember-product>article{position:sticky;top:0;align-self:start;display:grid;gap:1.2rem;min-height:100dvh;padding:clamp(1.5rem,4vw,4rem);background:var(--bone);color:var(--char)}.ember-product h1{font-size:clamp(3.5rem,7vw,7rem);margin:.5rem 0}.ember-variant{display:grid;grid-template-columns:1fr auto auto;gap:1rem;padding:.8rem 0;border-top:1px solid var(--ash)}@media(max-width:720px){.ember-product{grid-template-columns:1fr}.ember-product>article{position:static;min-height:auto}}` , "product"),
    },
    search: {
