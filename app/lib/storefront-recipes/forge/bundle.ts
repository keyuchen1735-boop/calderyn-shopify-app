...
export const FORGE_RECIPE_CONFIG = {
  ...
  surfaces: {
    ...
    collection: {
      ...
      source: source(
        `<main class="forge-frame forge-catalog"><header class="forge-catalog-head forge-copy"><span class="forge-label">Project index / merchant record</span><h1 data-cd-text="collection.title"></h1><p data-cd-text="collection.description"></p><b><span data-cd-text="collection.productCount"></span> live entries</b></header><nav class="forge-filters" aria-label="Project filters"><span>Project filters / live catalog tags</span><small>Only live products carrying the selected catalog tag appear.</small><button value="framing" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="tag">Framing</button><button value="finish" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="tag">Finish</button><button value="electrical" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="tag">Electrical</button><button value="repair" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="tag">Repair</button><button value="relevance" data-cd-on="click" data-cd-action="collection.sort">Drawing order</button><button value="price_asc" data-cd-on="click" data-cd-action="collection.sort">Price low</button><button value="price_desc" data-cd-on="click" data-cd-action="collection.sort">Price high</button></nav><div class="forge-grid" data-cd-repeat="collection.products">${productCard}</div><p class="forge-empty" data-cd-empty-state>No live products carry this catalog tag.</p></main><section data-cd-repeat="collection.products"><div data-cd-key="product.id" data-cd-slot="quickViewCommerce" data-cd-product="product.id" data-cd-host-size="inline" data-cd-theme-tokens="steel steel-dark orange parchment"></div></section>`,
