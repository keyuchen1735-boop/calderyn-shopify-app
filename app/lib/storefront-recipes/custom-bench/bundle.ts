...
const shell = {
  html: `
    <header class="workshop-shell">
      <a class="brand-mark" data-cd-route="home"><span data-cd-text="store.name"></span><b>Custom Bench</b></a>
  ...
  html: `
    <main class="bench-home">
      ...
      <section id="configurator" class="configurator" data-cd-state-id="bench-step" data-cd-state-type="enum" data-cd-state-initial="choose" data-cd-state-values="choose personalize approve" data-cd-bind-state="bench-step" data-cd-bind-property="classToken">
        ...
        <div class="step-rail">
          <button data-cd-on="click" data-cd-action="state.set" data-cd-state="bench-step">Choose the piece</button>
          <button data-cd-on="click" data-cd-action="state.set" data-cd-state="bench-step">Set your mark</button>
          <button data-cd-on="click" data-cd-action="state.set" data-cd-state="bench-step">Approve your proof</button>
        </div>
      </section>
      ...
      <aside data-cd-slot="quickViewCommerce" data-cd-host-size="panel" data-cd-theme-tokens="ink cobalt paper"></aside>
    </main>
  `,
  css: `
  ...
  html: `
    <main class="workshop-catalog">
      ...
      <aside class="filters" aria-label="Workshop filters">
        <button data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="material">Leather</button>
        <button data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="material">Paper</button>
        <button data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="availability">Ready soon</button>
        <button data-cd-on="click" data-cd-action="collection.sort">Sort by price</button>
      </aside>
...
const product = {
  html: `
    <main class="work-order">
      <section id="gallery" class="product-gallery"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><div data-cd-repeat="product.images"><button data-cd-key="product.primaryImage" data-cd-on="click" data-cd-action="gallery.select" data-cd-target="gallery"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"></button></div></section>
      <section class="purchase-sheet"><small>Workshop work order</small><h1 data-cd-text="product.title"></h1><p data-cd-money="product.price"></p><b data-cd-text="product.availability"></b><p class="recipe-copy" data-cd-text="product.description"></p><p>Sold out pieces remain visible with their workshop notes, but cannot be added.</p>
        <div class="variant-ledger" data-cd-repeat="product.variants"><button data-cd-key="variant.id" data-cd-text="variant.title" data-cd-on="click" data-cd-action="tabs.select" data-cd-target="variant-ledger"></button></div>
  ...
  css: `
    ...
    .variant-ledger button { background: transparent; border: 1px solid var(--rule); padding: .75rem }
...
const search = {
  html: `
    <main class="search-bench">
      ...
      <div class="query-controls" role="search"><button data-cd-on="input" data-cd-action="search.update">Update search</button><button data-cd-on="click" data-cd-action="search.submit">Search</button><button data-cd-on="click" data-cd-action="search.clear">Clear</button></div>
  ...
  css: `
    ...
    .query-controls button { background: transparent; border: 1px solid var(--rule); padding: .8rem 1rem }
