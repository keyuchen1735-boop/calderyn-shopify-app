...
const collections = route(
  `<main class="atelier-index"><header><p>Garment index</p><h1>Choose by use, then study the cut.</h1></header><nav aria-label="Collection index"><a data-cd-route="collection">Everyday layers</a><a data-cd-route="collection">Soft tailoring</a><a data-cd-route="collection">Foundation jerseys</a><a data-cd-route="collection">Warm-weather cloth</a></nav><section data-cd-repeat="featured.products"><article data-cd-key="product.id"><a data-cd-route="product" data-cd-param-handle="product.handle"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><h2 data-cd-text="product.title"></h2><span data-cd-money="product.price"></span></a></article></section><p class="atelier-empty" data-cd-empty-state>No garment studies are available.</p></main>`,
  `.atelier-index{min-height:80vh;padding:70px 28px;background:#d7d0c6}.atelier-index header{display:grid;grid-template-columns:1fr 2fr;gap:30px;border-bottom:1px solid}.atelier-index h1{max-width:12ch;margin:0 0 50px;font-family:var(--font-display);font-size:clamp(54px,8vw,110px);font-weight:500;line-height:.83}.atelier-index nav{display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid}.atelier-index nav a{padding:24px 12px;color:inherit;border-right:1px solid;text-decoration:none}.atelier-index section{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;padding-top:50px}.atelier-index article img{width:100%;aspect-ratio:4/5;object-fit:cover}.atelier-index article a{color:inherit;text-decoration:none}@media(max-width:760px){.atelier-index{padding:48px 16px}.atelier-index header,.atelier-index nav,.atelier-index section{grid-template-columns:1fr 1fr}}`,
);

const collection = route(
  `<main class="atelier-collection"><header><p><span data-cd-text="collection.productCount"></span> garment studies</p><h1 data-cd-text="collection.title"></h1><span data-cd-text="collection.description"></span></header><nav aria-label="Garment filters"><button value="all" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="category">All cuts</button><button value="close" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="fit">Close fit</button><button value="relaxed" data-cd-on="click" data-cd-action="collection.filter" data-cd-facet="fit">Relaxed fit</button><button value="price_asc" data-cd-on="click" data-cd-action="collection.sort">Price</button></nav><section class="atelier-collection-grid" data-cd-repeat="collection.products"><article data-cd-key="product.id"><a data-cd-route="product" data-cd-param-handle="product.handle"><img data-cd-src="product.primaryImage" data-cd-alt="product.title"><h2 data-cd-text="product.title"></h2><p data-cd-text="product.description"></p><b data-cd-money="product.price"></b><span data-cd-text="product.availability"></span></a></article></section><p class="atelier-empty" data-cd-empty-state>No garments match these fit notes.</p></main><section data-cd-repeat="collection.products"><div data-cd-key="product.id" data-cd-slot="quickViewCommerce" data-cd-product="product.id" data-cd-host-size="inline" data-cd-theme-tokens="stone oxblood ink"></div></section>`,
...
const product = route(
  `<main class="atelier-product"><section class="atelier-product-media"><img data-cd-src="product.primaryImage" data-cd-alt="product.title">${detailVideo.html}<div class="atelier-product-thumbs" data-cd-repeat="product.images"><img data-cd-key="product.primaryImage" data-cd-src="product.primaryImage" data-cd-alt="product.title"></div></section><article class="atelier-product-record"><p>Garment record / live variant</p><h1 data-cd-text="product.title"></h1><b data-cd-money="product.price"></b><span data-cd-text="product.availability"></span><p data-cd-text="product.description"></p><section class="atelier-measurements"><h2>Garment measurements</h2><p>Compare the merchant-authored garment facts with a piece you already wear well.</p><div class="atelier-variant-list" data-cd-repeat="product.variants"><div class="atelier-variant-row" data-cd-key="variant.id"><b data-cd-text="variant.title"></b><span data-cd-text="variant.availability"></span></div></div></section><section id="atelier-fit-finder" class="atelier-fit-finder" data-cd-state-id="fit-profile" data-cd-state-type="enum" data-cd-state-initial="regular" data-cd-state-values="close regular relaxed" data-cd-bind-state="fit-profile" data-cd-bind-property="classToken"><h2>Fit finder</h2><p>How should this garment sit on the body?</p><div><button value="close" data-cd-on="click" data-cd-action="state.set" data-cd-state="fit-profile">Close</button><button value="regular" data-cd-on="click" data-cd-action="state.set" data-cd-state="fit-profile">Regular</button><button value="relaxed" data-cd-on="click" data-cd-action="state.set" data-cd-state="fit-profile">Relaxed</button></div><div id="atelier-measurement-step" data-cd-state-id="measurement-step" data-cd-state-type="index" data-cd-state-initial="0" data-cd-state-min="0" data-cd-state-max="3" data-cd-bind-state="measurement-step" data-cd-bind-property="activeIndex"><p>Body-aware measurement flow: shoulder, chest, and preferred ease.</p><button value="back" data-cd-on="click" data-cd-action="state.decrement" data-cd-state="measurement-step">Previous measure</button><button value="next" data-cd-on="click" data-cd-action="state.increment" data-cd-state="measurement-step">Next measure</button></div><aside class="atelier-confidence"><h3>Size confidence</h3><p>Recommendation withheld when measurements are incomplete.</p><p>A supported result names the garment facts used; the live variant picker remains purchase authority.</p></aside></section></article></main><section><div data-cd-slot="variantPicker" data-cd-product="product.id" data-cd-host-size="block" data-cd-theme-tokens="stone oxblood ink"></div>${stickyPurchase.html}</section>`,
  `.atelier-product{display:grid;grid-template-columns:1.2fr .8fr;min-height:100vh;background:#ebe6dd}.atelier-product-media{position:sticky;top:72px;align-self:start;display:grid;grid-template-columns:2fr 1fr;gap:8px;padding:16px}.atelier-product-media>img{grid-row:span 2;width:100%;height:78vh;object-fit:cover}.atelier-detail-video{width:100%;height:38vh;object-fit:cover}.atelier-product-thumbs{display:grid;gap:8px}.atelier-product-thumbs img{width:100%;height:19vh;object-fit:cover}.atelier-product-record{padding:70px 6vw 130px;background:#d7d0c6}.atelier-product-record>p:first-child{font-size:11px;letter-spacing:.12em;text-transform:uppercase}.atelier-product-record h1{margin:16px 0;font-family:var(--font-display);font-size:clamp(54px,7vw,104px);font-weight:500;line-height:.82}.atelier-product-record>p{line-height:1.65}.atelier-measurements,.atelier-fit-finder{margin-top:34px;padding-top:24px;border-top:1px solid}.atelier-measurements h2,.atelier-fit-finder h2{font-family:var(--font-display);font-size:32px;font-weight:500}.atelier-variant-row{display:flex;justify-content:space-between;padding:10px 0;border-top:1px solid #7b716a}.atelier-fit-finder button{min-height:44px;margin:4px;padding:10px 14px;border:1px solid;background:transparent}.atelier-fit-finder button:hover{background:#681f2b;color:#fff}.atelier-confidence{margin-top:20px;padding:18px;background:#ebe6dd;border-left:5px solid #681f2b}@media(max-width:900px){.atelier-product{grid-template-columns:1fr}.atelier-product-media{position:static}.atelier-product-record{padding:48px 16px 100px}.atelier-detail-video{height:32vh}}@media(prefers-reduced-motion:reduce){.atelier-product-media{position:static}.atelier-detail-video{animation:none}}`,
  "product",
);
...
const story = route(
  `<main class="atelier-story"><header><p>Fit method</p><h1>Measure the garment. Respect the body.</h1></header><section><article><b>01</b><h2>Start with cloth</h2><p>Fabric weight, stretch, and recovery change how a measurement feels in motion.</p></article><article><b>02</b><h2>Compare a known piece</h2><p>Use a garment you already wear well instead of treating one body number as the whole answer.</p></article><article><b>03</b><h2>State confidence</h2><p>When merchant measurements are incomplete, the fit finder withholds a recommendation.</p></article></section><a data-cd-route="collection">Open the garment laboratory</a></main>`,
...
export const ATELIER_RECIPE_CONFIG = {
  ...
  concept: {
    ...
    rationale:
      "A calm garment laboratory turns live apparel variants into a measured fit decision without editorial-magazine theatrics.",
    noveltySignature: [
      "body-aware fit finder",
      "merchant-grounded confidence record",
      "calm moving fabric studies",
      "sticky live-variant purchase",
    ],
  },
  designSystem: {
  ...
  surfaces: {
    ...
    collection: {
      signature:
        "fit-filter laboratory header above an even garment measurement matrix",
      source: collection,
    },
    product: {
      signature:
        "sticky fabric bench beside body-aware fit evidence and live variant purchase",
      source: product,
    },
    search: {
      signature:
