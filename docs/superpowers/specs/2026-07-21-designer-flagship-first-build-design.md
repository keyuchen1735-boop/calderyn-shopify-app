...
- **Registry validation**: every image reference in generated documents must be either a known placeholder binding, a real asset path from the donor template's manifest, or an approved neutral. Invented paths (walkthrough: `/storefront-recipes/candle-*.jpg`, 404) are an audit fail (shared mechanism with D2.3). Ready-asset checks must filter `store_asset` to `status='ready' AND url <> ''` — failed generation rows have empty URLs and must never count as usable photos (2026-07-21 recon correction: the walkthrough shop's 3 `store_asset` rows were failed attempts, not photos; the catalog genuinely had zero images).
- **Reuse before generate**: first builds source imagery from what the shop already has (ready product media / ready store assets / logo) before spending generation quota; a hero sourced from an existing photo costs zero quota.
- **Mismatch rule / donor-art replacement**: on a first build, donor-template art must be replaced (with a placeholder binding to real imagery) or removed — never kept as-is (the salmon photo survived with a dishonest candle alt-text). Donor paths already present in previously saved documents stay legal for edit turns, so existing stores don't break.
- **Reserved first-build budget**: the designer's first build gets a reserved allowance layered on the shared daily image cap (limits are parameters into the existing reservation RPC — override at the designer call sites; do NOT change the RPC's counting semantics, which deliberately count provider-failed attempts to prevent retry-storm spend). Recon caveat: the walkthrough's quota was burned by nine provider-throttled *failures*, so the reserve alone would not have saved that build — bounded, in-slot retry on provider throttle is a separate, carefully-scoped item.
